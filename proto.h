...
static inline double ForceSoftening_KernelRadius(int p)
{
    ...
    if(((1 << P[p].Type) & (ADAPTIVE_GRAVSOFT_FROM_TIDAL_CRITERION)) && (P[p].tidal_tensor_mag_prev>0 && All.Time>All.TimeBegin)) {return DMIN(1.e6*All.ForceSoftening[P[p].Type] , DMAX(All.ForceSoftening[P[p].Type] , 2.202 * pow( (All.DesNumNgb * All.G * P[p].Mass / P[p].tidal_tensor_mag_prev) , 1./3. )));} else {return 100.*All.ForceSoftening[P[p].Type];}
#endif
