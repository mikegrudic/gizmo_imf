...
{
  pid_t my_pid;
  char buf[DEFAULT_PATH_BUFFERSIZE_TOUSE], hostname[DEFAULT_PATH_BUFFERSIZE_TOUSE], *cp;
  char commandbuf[DEFAULT_PATH_BUFFERSIZE_TOUSE];
  FILE *fd;
  int i, pid;

  snprintf(buf, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s%s", All.OutputDir, "PIDs.txt");
  ...
  if((fd = fopen(buf, "r")))
      ...
      for(i = 0; i < NTask; i++)
	{
	  ...
	  if(my_pid != pid)
	    {
	      snprintf(commandbuf, DEFAULT_PATH_BUFFERSIZE_TOUSE, "ssh %s kill -ABRT %d", hostname, pid);
	      printf("--> %s\n", commandbuf);
	      fflush(stdout);
...
{
  pid_t my_pid; char mode[8], buf[DEFAULT_PATH_BUFFERSIZE_TOUSE]; FILE *fd; int i;
  my_pid = getpid();
  snprintf(buf, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s%s", All.OutputDir, "PIDs.txt");
  ...
  for(i = 0; i < NTask; i++)
    {
      if(ThisTask == i)
	{
	  if(ThisTask == 0) {snprintf(mode, 8, "w");} else {snprintf(mode, 8, "a");}
