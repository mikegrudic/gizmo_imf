...
Required initializations outside this block:
    ROOTFIND_FUNCTION(x) - #define'd macro that evaluates a function of a single variable whose root we wish to find
    ROOTFIND_X_a, ROOTFIND_X_b - Arguments to ROOTFIND_FUNCTION that are known to bracket the root.
    ROOTFIND_REL_X_tol - Tolerance for desired *relative* error in the root
*/


if (fabs(ROOTFUNC_a) < fabs(ROOTFUNC_b))
{ // in our convention 'a' represents the bracket with the larger residual
...
double ROOTFIND_X_c_old = ROOTFIND_X_c, ROOTFIND_X_new, ROOTFUNC_new = ROOTFUNC_c, ROOTFIND_REL_X_error = 1e100, EPS_TOL=1e-8;

/* now we do a Brent 1973 method root-find */
while (ROOTFIND_REL_X_error > ROOTFIND_REL_X_tol)
{
    ROOTFIND_X_new = 0;
    if ((ROOTFUNC_a != ROOTFUNC_c) && (ROOTFUNC_b != ROOTFUNC_c))
    ...
    if (DO_BISECTION)
    {
	// bisection in log space can help convergence for typical use cases; do this if possible
