...
double CosmicRay_Update_DriftKick(int i, double dt_entr, int mode)
{
    ...
    for(q_whichupdate=0; q_whichupdate<q_N_updates; q_whichupdate++)
    {
        ...
        double dCR_dt = CellP.DtCosmicRayEnergy[i][k_CRegy], gamma_eff = GAMMA_COSMICRAY(k_CRegy), eCR_tmp = eCR;
        if(q_whichupdate>0) {dCR_dt=CellP.DtCosmicRayAlfvenEnergy[i][k_CRegy][q_whichupdate-1]; gamma_eff=(3./2.); if(mode==0) {eCR_tmp=CellP.CosmicRayAlfvenEnergy[i][k_CRegy][q_whichupdate-1];} else {eCR_tmp=CellP.CosmicRayAlfvenEnergyPred[i][k_CRegy][q_whichupdate-1];}}
        ...
        if(q_whichupdate==0) {if(mode==0) {CellP.CosmicRayEnergy[i][k_CRegy]=eCR_tmp;} else {CellP.CosmicRayEnergyPred[i][k_CRegy]=eCR_tmp;}} // updated energy
        if(q_whichupdate>0) {if(mode==0) {CellP.CosmicRayAlfvenEnergy[i][k_CRegy][q_whichupdate-1]=eCR_tmp;} else {CellP.CosmicRayAlfvenEnergyPred[i][k_CRegy][q_whichupdate-1]=eCR_tmp;}} // updated energy
        // now need to account for the adiabatic heating/cooling of the 'fluid', here, with gamma=gamma_eff //
        double eCR_0 = eCR_tmp, d_div = (-(gamma_eff-1.) * CellP.Face_DivVel_ForAdOps[i]*All.cf_a2inv) * dt_entr;
        ...
        double dCR_div = DMIN(eCR_tmp*d_div , 0.5*u0*P.Mass[i]); // limit so don't take away all the gas internal energy [to negative values]
        ...
        if(dCR_div < -0.5*P.Mass[i]*u0) {dCR_div=-0.5*P.Mass[i]*u0;} // before re-coupling, ensure this will not cause negative energies
        if(dCR_div < -0.9*eCR_00) {dCR_div=-0.9*eCR_00;} // before re-coupling, ensure this will not cause negative energies 
        if(q_whichupdate==0) {if(mode==0) {CellP.CosmicRayEnergy[i][k_CRegy] += dCR_div; CellP.InternalEnergy[i] -= dCR_div/P.Mass[i];} else {CellP.CosmicRayEnergyPred[i][k_CRegy] += dCR_div; CellP.InternalEnergyPred[i] -= dCR_div/P.Mass[i];}}
        if(q_whichupdate>0) {if(mode==0) {CellP.CosmicRayAlfvenEnergy[i][k_CRegy][q_whichupdate-1] += dCR_div; CellP.InternalEnergy[i] -= dCR_div/P.Mass[i];} else {CellP.CosmicRayAlfvenEnergyPred[i][k_CRegy][q_whichupdate-1] += dCR_div; CellP.InternalEnergyPred[i] -= dCR_div/P.Mass[i];}}
    }
    ...
    double ne=1, f_ion=1, nh0=0, nHe0, nHepp, nhp, nHeII, temperature, mu_meanwt=1, rho=CellP.Density[i]*All.cf_a3inv, rho_cgs=rho*UNIT_DENSITY_IN_CGS;
#ifdef COOLING 
...
#endif
    for(k=0;k<3;k++) {bhat[k] = (mode==0) ? CellP.B[i][k] : CellP.BPred[i][k];} // grab whichever B field we need for our mode
    if(mode==0) {eCR=CellP.CosmicRayEnergy[i][k_CRegy]; u0=CellP.InternalEnergy[i];} else {eCR=CellP.CosmicRayEnergyPred[i][k_CRegy]; u0=CellP.InternalEnergyPred[i];} // initial energy
    if(u0<All.MinEgySpec) {u0=All.MinEgySpec;} // enforce the usual minimum thermal energy the code requires
    for(k=0;k<2;k++) {if(mode==0) {eA[k]=CellP.CosmicRayAlfvenEnergy[i][k_CRegy][k];} else {eA[k]=CellP.CosmicRayAlfvenEnergyPred[i][k_CRegy][k];}} // Alfven energy
    for(k=0;k<3;k++) {flux[k] = (mode==0) ? CellP.CosmicRayFlux[i][k_CRegy][k] : CellP.CosmicRayFluxPred[i][k_CRegy][k];} // load flux
    ...
    Bmag *= CellP.Density[i]/P.Mass[i] * All.cf_a2inv; // convert to actual B in physical units
    E_B = 0.5*Bmag*Bmag * (P.Mass[i]/(CellP.Density[i]*All.cf_a3inv)); // B-field energy (energy density times volume, for ratios with energies above)
    double Eth_0 = EPSILON_SMALL + 1.e-8 * P.Mass[i]*u0; // set minimum magnetic energy relative to thermal (maximum plasma beta ~ 1e8) to prevent nasty divergences
    if(E_B < Eth_0) {Bmag = sqrt(2.*Eth_0/((P.Mass[i]/(CellP.Density[i]*All.cf_a3inv))));} // enforce this maximum beta for purposes of "B" to insert below 
    E_B = 0.5*Bmag*Bmag * (P.Mass[i]/(CellP.Density[i]*All.cf_a3inv)); // B-field energy (energy density times volume, for ratios with energies above)
    ...
    r_turb_driving = CellP.Gradients.Pressure[i].norm_sq(); // compute gradient magnitude
    r_turb_driving = DMAX( CellP.Pressure[i] / (EPSILON_SMALL + sqrt(r_turb_driving)) , Get_Particle_Size(i) ) * All.cf_atime; // maximum of gradient scale length or resolution scale
    ...
    fac=0; for(k=0;k<3;k++) {fac += All.cf_a2inv * bhat[k] * (bhat[0]*CellP.Gradients.Velocity[i][k][0] + bhat[1]*CellP.Gradients.Velocity[i][k][1] + bhat[2]*CellP.Gradients.Velocity[i][k][2]);}
    ...
    { 
        double h=Get_Particle_Size(i)*All.cf_atime; int k2; for(k=0;k<3;k++) {for(k2=0;k2<3;k2++) {Min_Egy+=CellP.Gradients.B[i][k][k2]*CellP.Gradients.B[i][k][k2];}}
        ...
        Min_Egy = 0.5 * (Min_Egy*Min_Egy) * P.Mass[i]/(CellP.Density[i]*All.cf_a3inv); // magnetic energy at this scale, from the above //
        ...
        if(Min_Egy <= 0 || !isfinite(Min_Egy)) {Min_Egy = 1.e-15*P.Mass[i]*u0;} // if this minimum-energy calculation failed, enforce a tiny fraction of the thermal energy
    ...
    double eCR_0 = 1.e-6*(E_B + P.Mass[i]*u0) + eCR + eA[0] + eA[1] + fabs(f_CR/cr_speed); // this can be anything, just need a normalization for the characteristic energy scale of the problem //
    double ceff2_va2=(cr_speed*cr_speed)/(vA_code*vA_code), t0=1./(fac_Omega*(eCR_0/E_B)*vA2_c2), gammCR=GAMMA_COSMICRAY(k_CRegy), f_unit=vA_code*eCR_0, volume=P.Mass[i]/(CellP.Density[i]*All.cf_a3inv); // factors used below , and for units
    ...
    flux_G = dot(bhat, CellP.Gradients.CosmicRayPressure[i][k_CRegy]); // b.gradient[P] -- flux source term
    ...
    for(i1=0;i1<3;i1++)
    {
        v2_t += CellP.VelPred[i][i1]*CellP.VelPred[i][i1]; b2_t += Get_Gas_BField(i,i1) * Get_Gas_BField(i,i1);
        for(i2=0;i2<3;i2++) {dv2_t += CellP.Gradients.Velocity[i][i1][i2]*CellP.Gradients.Velocity[i][i1][i2]; db2_t += CellP.Gradients.B[i][i1][i2]*CellP.Gradients.B[i][i1][i2];}
    }
    ...
    if(mode==0) {CellP.CosmicRayEnergy[i][k_CRegy]=eCR;} else {CellP.CosmicRayEnergyPred[i][k_CRegy]=eCR;} // CR energy
    for(k=0;k<2;k++) {if(mode==0) {CellP.CosmicRayAlfvenEnergy[i][k_CRegy][k]=eA[k];} else {CellP.CosmicRayAlfvenEnergyPred[i][k_CRegy][k]=eA[k];}} // Alfven energy
    {Vec3<double> flux_out = bhat*f_CR; if(mode==0) {for(k=0;k<3;k++) {CellP.CosmicRayFlux[i][k_CRegy][k]=flux_out[k];}} else {for(k=0;k<3;k++) {CellP.CosmicRayFluxPred[i][k_CRegy][k]=flux_out[k];}}} // assign to flux vector
    if(mode==0) {CellP.InternalEnergy[i]+=thermal_heating/P.Mass[i];} else {CellP.InternalEnergyPred[i]+=thermal_heating/P.Mass[i];} // heating term from damping
    CellP.CosmicRayDiffusionCoeff[i][k_CRegy] = 1. / (fac_Omega*((eA[0]+eA[1])/E_B)/(clight_code*clight_code)); // effective diffusion coefficient in code units
