    ...
    for(i = 0; i < NumPart; i++)	/*  start-up initialization */
    {
        ...
        if((RestartFlag == 0) && ((1 << P[i].Type) & (GRAIN_PTYPES)))
        {
            ...
            if(grain_subtype <= 2)
            {
                ...
                double q_a = (0.75*All.Grain_Q_at_MaxGrainSize) / (All.Grain_Internal_Density*All.Grain_Size_Max), kappa_0 = All.Grain_Absorbed_vs_Total_Extinction * q_a * All.Dust_to_Gas_Mass_Ratio;
