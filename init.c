    ...
    for(i = 0; i < NumPart; i++)	/*  start-up initialization */
    {
        ...
        if((RestartFlag == 0) && ((1 << P[i].Type) & (GRAIN_PTYPES)))
        {
            ...
            if(grain_subtype <= 2)
            {
                ...
                rho_gas_expected = 1.*UNIT_DENSITY_IN_CGS; /* guess for the gas density here [set custom for e.g. stratified problems */
                tS0 = (0.626657 * P[i].Grain_Size * sqrt(GAMMA_DEFAULT) / rho_gas_expected) / UNIT_TIME_IN_CGS; /* stopping time [Epstein] for driftvel->0 [cgs->code units] */
                ...
                a0 = tS0 * amag / (1.+All.Dust_to_Gas_Mass_Ratio); /* acc * tS0 / (1+mu) -- we're assuming that the code unit velocity equals the sound speed, for simplicity here */
