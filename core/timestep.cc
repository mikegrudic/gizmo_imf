...
#pragma omp parallel for schedule(dynamic) reduction(max:fastwavespeed,fastwavedecay)
        ...
        for(i=0;i<NumPart;i++)
        {
            if(P.Type[i]==0)
            {
                double vsig2 = 0.5  * fabs(CellP.MaxSignalVel[i]); // in v_phys units //
                double vsig1 = sqrt( Get_Gas_effective_soundspeed_i(i)*Get_Gas_effective_soundspeed_i(i) + fac_magnetic_pressure * (Get_Gas_BField(i,0)*Get_Gas_BField(i,0)+Get_Gas_BField(i,1)*Get_Gas_BField(i,1)+Get_Gas_BField(i,2)*Get_Gas_BField(i,2)) / CellP.Density[i] );
    ...
    for (int i : ActiveParticleList)
    {
        ...
        if(is_particle_a_special_zoom_target(i)==0) {ti_step = P.dt_step[i];} else {ti_step = TIMEBASE;} // set the source particle to have a timestep no more than 4 bins larger than the previous smallest active particle/cell bin timestep
#endif
    ...
    for (int i : ActiveParticleList)
    {
        ...
        if(bin != binold)
        {
            ...
            if(P.Type[i] == 5)
            {
                TimeBin_Sink_mass[binold] -= P.Sink_Mass[i];
                TimeBin_Sink_dynamicalmass[binold] -= P.Mass[i];
                TimeBin_Sink_Mdot[binold] -= P.Sink_Mdot[i];
                if(P.Sink_Mass[i] > 0) {TimeBin_Sink_Medd[binold] -= P.Sink_Mdot[i] / P.Sink_Mass[i];}
                TimeBin_Sink_mass[bin] += P.Sink_Mass[i];
                TimeBin_Sink_dynamicalmass[bin] += P.Mass[i];
                TimeBin_Sink_Mdot[bin] += P.Sink_Mdot[i];
                if(P.Sink_Mass[i] > 0) {TimeBin_Sink_Medd[bin] += P.Sink_Mdot[i] / P.Sink_Mass[i];}
            }
#endif
            ...
            TimeBinCount[bin]++;
            if(P.Type[i] == 0) {TimeBinCountGas[bin]++;}
            P.TimeBin[i] = bin;
        }
...
#else
        ti_step_old = P.dt_step[i];
#endif
        P.Ti_begstep[i] += ti_step_old;
#if defined(WAKEUP)
        P.dt_step[i] = ti_step;
#endif
        ...
        if(P.Type[i] == 5){
            if(All.Ti_Current == 0) { // first timestep
                P.dt_since_last_gas_search[i] = GET_PHYSICAL_TIMESTEP_FROM_TIMEBIN(P.TimeBin[i],i);
                P.do_gas_search_this_timestep[i] = 1;
            } else {
                P.dt_since_last_gas_search[i] += GET_PHYSICAL_TIMESTEP_FROM_TIMEBIN(P.TimeBin[i],i);
                if(P.dt_since_last_gas_search[i] > 0.49 * GET_PHYSICAL_TIMESTEP_FROM_TIMEBIN(P.Sink_TimeBinGasNeighbor[i],i)){
                    P.do_gas_search_this_timestep[i] = 1;
                } else {P.do_gas_search_this_timestep[i] = 0;}
            }
#if defined(SINGLE_STAR_STARFORGE_PROTOSTELLAR_EVOLUTION)
	    if(P.ProtoStellarStage[i] == 6) {P.do_gas_search_this_timestep[i] = 1;} // always do gas search if we're rapidly spawning in new gas shells
#endif
        }
#endif
        
#ifdef SINGLE_STAR_AND_SSP_NUCLEAR_ZOOM
        if(is_particle_a_special_zoom_target(i) && P.Mass[i] > 0) {xyz_local[j_specialpartical_counter][0]=P.Pos[i][0]; xyz_local[j_specialpartical_counter][1]=P.Pos[i][1]; xyz_local[j_specialpartical_counter][2]=P.Pos[i][2]; special_particle_active_with_this_index[j_specialpartical_counter]=i; special_particle_mass_local[j_specialpartical_counter]=P.Mass[i]; j_specialpartical_counter++;} // active on this processor, set
#endif
    ...
    for(k=0;k<SINGLE_STAR_AND_SSP_NUCLEAR_ZOOM;k++)
    {
        if(xyz_global[k][0] > -1.e10) { // this indicates that the special particle was active on one task
            ...
            if(special_particle_active_with_this_index[k]>=0) {P.Mass[special_particle_active_with_this_index[k]] += mass_to_sum_global[k]; special_particle_mass_local[k] += mass_to_sum_global[k];} // the special particle lives here with this id, so we can update it with this mass
                         ...
                         int flag	/*!< either 0 for normal operation, or finite timestep to get corresponding aphys */ )
{
    ...
    P.SuperTimestepFlag[p] = 0;
    if( (P.Type[p] == 5) && P.is_in_a_binary[p] ) // candidate: need to decide whether to use super timestepping for binaries
    {
#if (SINGLE_STAR_TIMESTEPPING == 1) // to be conservative, use the semimajor axis, ie. the internal timescale is the orbital period
	    double dt_bin = P.Min_Sink_OrbitalTime[p] / (2.*M_PI); // sqrt(a^3/GM) for binary
	    if(0.03*P.COM_dt_tidal[p]>dt_bin) {P.SuperTimestepFlag[p]=2;
	    ...
	    double dr = P.comp_dx[p].norm();
	    double dt_bin = sqrt(dr*dr*dr / (All.G * (P.Mass[p] + P.comp_Mass[p])));
        if(0.005*P.COM_dt_tidal[p]>dt_bin) {P.SuperTimestepFlag[p]=2;} // external timestep is appropriately larger than 'internal' timestep, so use super-timestepping routine [constant here stricter for more aggressive routine]
#endif
    }
#endif
...
#endif
        {
            Vec3<double> acc = P.GravAccel[p] * All.cf_a2inv;
#ifdef PMGRID
            acc += P.GravPM[p] * All.cf_a2inv;
#endif
            if(P.Type[p]==0) {
                acc += CellP.HydroAccel[p];
#ifdef TURB_DRIVING
                acc += CellP.TurbAccel[p];
#endif
#ifdef RT_RAD_PRESSURE_OUTPUT
                acc += CellP.Rad_Accel[p];
#endif
            }
            P.Acc_Total_PrevStep[p] = acc;
        }
    }
#endif
    ...
    if(flag == 0)
    {
        ax = All.cf_a2inv * P.GravAccel[p][0];
        ay = All.cf_a2inv * P.GravAccel[p][1];
        az = All.cf_a2inv * P.GravAccel[p][2];
#ifdef PMGRID
        ax += All.cf_a2inv * P.GravPM[p][0];
        ay += All.cf_a2inv * P.GravPM[p][1];
        az += All.cf_a2inv * P.GravPM[p][2];
#endif
        
#if defined(TIDAL_TIMESTEP_CRITERION)
#if defined(RT_USE_GRAVTREE) && !defined(SINGLE_STAR_FB_RT_HEATING)
        if(P.Type[p]>0) // strictly this is better for accuracy, but not necessary
#endif
        ...
        if(P.Type[p] == 0)
        {
            ax += CellP.HydroAccel[p][0];
            ay += CellP.HydroAccel[p][1];
            az += CellP.HydroAccel[p][2];
#ifdef TURB_DRIVING
            ax += CellP.TurbAccel[p][0];
            ay += CellP.TurbAccel[p][1];
            az += CellP.TurbAccel[p][2];
#endif
#ifdef RT_RAD_PRESSURE_OUTPUT
            ax += CellP.Rad_Accel[p][0];
            ay += CellP.Rad_Accel[p][1];
            az += CellP.Rad_Accel[p][2];
#endif
        }
...
#ifdef GRAIN_FLUID
    if(((1 << P.Type[p]) & (GRAIN_PTYPES)) && (h_for_accel_dt <= 0)) {h_for_accel_dt = Get_Particle_Size(p) * All.cf_atime;} /* for grain particles without gravity, use the inter-particle spacing as the characteristic length scale */
#endif
    ...
    if(((P.Type[p] == 4)||((All.ComovingIntegrationOn==0)&&((P.Type[p] == 2)||(P.Type[p]==3))))&&(P.Mass[p]>0))
    {
        if((All.ComovingIntegrationOn)) // sort of a hack here, but acceptable in applications
        {
            double h_min = All.ForceSoftening[P.Type[p]], ags_h = DMIN(DMAX(P.KernelRadius[p], h_min), 10.*h_min);
#ifdef ADAPTIVE_GRAVSOFT_FORALL
            ags_h = DMIN(DMAX(P.AGS_KernelRadius[p] , DMAX(P.KernelRadius[p], h_min)) , DMAX(100.*h_min, 10.*P.AGS_KernelRadius[p]));
#endif
    ...
    double tidal_mag = P.tidal_tensorps[p].frobenius_norm(); // can estimate time derivative here, via: dt_ttmag = (tidal_mag-P.tidal_tensor_mag_prev[p]) / GET_PARTICLE_TIMESTEP_IN_PHYSICAL(p);
    double dt_tidalsoft = All.CourantFac * NUMDIMS * DMAX(DMAX(GET_PARTICLE_TIMESTEP_IN_PHYSICAL(p), dt), All.MinSizeTimestep) * (tidal_mag+P.tidal_tensor_mag_prev[p]) / (fabs(tidal_mag-P.tidal_tensor_mag_prev[p]) + MIN_REAL_NUMBER);
    if(((1 << P.Type[p]) & (ADAPTIVE_GRAVSOFT_FROM_TIDAL_CRITERION)) && (P.tidal_tensor_mag_prev[p]>0 && All.Time>All.TimeBegin)) {dt = DMIN(dt, dt_tidalsoft);} // use as a timestep criterion for tidal-ags-active particles
    P.tidal_tensor_mag_prev[p] = tidal_mag; // save it (overwriting previous value)
    {
        double tt2 = P.tidal_tensorps[p].frobenius_norm_sq();
        double tracett = P.tidal_tensorps[p].trace(); /* compute numbers needed below */
        double H_eff = ForceSoftening_KernelRadius(p); /* get value to calculate H we need to use in the equations below */
        if(tidal_mag > 0) {P.tidal_zeta[p] *= -All.G*(H_eff-All.ForceSoftening[P.Type[p]])/(2.*NUMDIMS*tt2 - 0 * 8.*M_PI*tracett*(All.G*P.Mass[p]/pow(H_eff,NUMDIMS)));} else {P.tidal_zeta[p] = 0;}
        P.tidal_tensorps_prevstep[p] = P.tidal_zeta[p] * P.tidal_tensorps[p]; /* save for next iteration in gravtree */
    }
#endif
    ...
    double tidal_mag_dt = P.tidal_tensorps[p].frobenius_norm_sq();
    ...
    if(P.Type[p] == 0) {dt_tidal = DMIN(sqrt(All.ErrTolIntAccuracy/(All.G*CellP.Density[p]*All.cf_a3inv)), dt_tidal);} // gas self-gravity timescale as a bare minimum
    ...
    P.tdyn_step_for_treeforce[p] = dt_tidal; // hang onto this to decide how frequently to update the treeforce
#endif
    
#if (SINGLE_STAR_TIMESTEPPING > 0)
    if(P.SuperTimestepFlag[p]>=2) {dt_tidal = sqrt(2*All.ErrTolIntAccuracy) * P.COM_dt_tidal[p];}
#endif
    dt=DMIN(dt,dt_tidal);
#endif
    ...
    if(P.Type[p] == 5)
    {
        double dt_2body = sqrt(2*All.ErrTolIntAccuracy) * 0.3 / (1./P.Min_Sink_Approach_Time[p] + 1./P.Min_Sink_Freefall_time[p]); // timestep is harmonic mean of freefall and approach time
#ifdef HERMITE_INTEGRATION
        if(eligible_for_hermite(p)) dt_2body /= 0.3;
#endif
#if (SINGLE_STAR_TIMESTEPPING > 0)
    	if(P.is_in_a_binary[p] && (P.SuperTimestepFlag[p] >= 2)) //binary candidate or a confirmed binary
	    {    // First we need to construct the same 2-body timescale as above, but from the binary parameters. If this is longer than the above, there is another star that is requiring us to
	        ...
	        double Mtot=P.comp_Mass[p]+P.Mass[p], dr=P.comp_dx[p].norm_sq(), dv=P.comp_dv[p].norm_sq(), dv_dot_dx=dot(P.comp_dx[p],P.comp_dv[p]), binary_dt_2body=0;
            ...
            double dt_2body_base = 1/(1./P.Min_Sink_Approach_Time[p] + 1./P.Min_Sink_Freefall_time[p]); // timestep is harmonic mean of freefall and approach time
	        ...
	        } else {P.SuperTimestepFlag[p] = 0;}  // we still have to take a proper short N-body integration timestep due to a third body whose approach requires careful integration, so no super timestepping is possible
	    }
#endif
        dt = DMIN(dt, dt_2body);
#ifdef HERMITE_INTEGRATION
    ...
    if(P.Type[p] == 0) {dt = DMIN(dt, 0.5 * All.CourantFac * DMIN(P.Min_Sink_FeedbackTime[p], P.Min_Sink_Approach_Time[p]));}
#endif    
#endif // SINGLE_STAR_TIMESTEPPING

#ifdef ADAPTIVE_GRAVSOFT_FORALL
    /* make sure smoothing length of non-gas particles doesn't change too much in one timestep */
    if(((1 << P.Type[p]) & (ADAPTIVE_GRAVSOFT_FORALL)) && (P.Type[p] > 0))
    {
        double dt_divv = 0.1 / (MIN_REAL_NUMBER + All.cf_a2inv*fabs(P.Particle_DivVel[p])); // with new integration accuracy in gravtree, we may not need to be super-conservative here. old code used pre-factor 0.25 here, see if we can get away with the larger value which is standard for gas below
        if(dt_divv < dt) {dt = dt_divv;}
        double dt_cour = 2. * All.CourantFac * (Get_Particle_Size_AGS(p)*All.cf_atime) / (MIN_REAL_NUMBER + 0.5*P.AGS_vsig[p]); // can be generous here, really the signal velocity isn't that important in the collisionless case, but it is important with some of the physics above //
...
#ifdef DM_FUZZY
    if((P.Type[p] > 0) && (P.AGS_Density[p] > 0))
    {
        ...
        dt_cour_ags_fuzzy = 0.25 * L_particle_ags_x / sqrt(MIN_REAL_NUMBER + (10./9.)*P.AGS_Numerical_QuantumPotential[p]/P.Mass[p]); // wavespeed based on 'stored' sub-grid energy [can get comparable]
        if(dt_cour_ags_fuzzy < dt) {dt = dt_cour_ags_fuzzy;}
    }
#endif
...
#ifdef GRAIN_FLUID
    if((1 << P.Type[p]) & (GRAIN_PTYPES))
    {
        csnd = convert_internalenergy_soundspeed2(p, P.Gas_InternalEnergy[p]);
        csnd += (P.Gas_Velocity[p] - P.Vel[p]).norm_sq();
#if defined(GRAIN_LORENTZFORCE)
        csnd += P.Gas_B[p].norm_sq() / (2.0 * P.Gas_Density[p]);
#endif
        csnd = sqrt(csnd);
        double L_particle = Get_Particle_Size(p);
        dt_courant = 0.5 * All.CourantFac * (L_particle*All.cf_atime) / csnd;
#if defined(GRAIN_BACKREACTION)
        if(6.*P.Grain_AccelTimeMin[p] < dt_courant) {dt_courant = 6.*P.Grain_AccelTimeMin[p];}
#endif
#if defined(GRAIN_LORENTZFORCE) && defined(GRAIN_RDI_TESTPROBLEM)
        if(All.Grain_Charge_Parameter != 0) {double bmag = P.Gas_B[p].norm_sq();
...
#endif
            double B2=(P.Gas_B[p] * All.cf_a2inv).norm_sq(), beta2=(P.Vel[p] * (1.0/(All.cf_atime*reduced_C))).norm_sq(); /* get magnitude and unit vector for B, and vector beta [-true- beta here] */
    ...
    if(P.Type[p]>-1) {double dt_inj = 0.1 * P.KernelRadius[p] / C_LIGHT_CODE_REDUCED(p); if(P.Type[p]==4) {dt_inj*=0.25;} if(dt_inj < dt) {dt = dt_inj;}}
#endif
#endif


    if((P.Type[p] == 0) && (P.Mass[p] > 0))
        {
            csnd = 0.5 * CellP.MaxSignalVel[p] ;
            double L_particle = Get_Particle_Size(p);
            dt_courant = All.CourantFac * (L_particle*All.cf_atime) / csnd;
#if defined(SINK_WIND_SPAWN) && !defined(SINK_RIAF_SUBEDDINGTON_MODEL)
            if(P.ID[p] == All.SpawnedWindCellID) {dt_courant *= 0.5;} // be more careful if this is a spawned-in gas cell
#endif
            if(dt_courant < dt) dt = dt_courant;
...
#ifdef CONDUCTION
            {
                double L_cond_inv = sqrt(CellP.Gradients.InternalEnergy[p][0]*CellP.Gradients.InternalEnergy[p][0] +
                                         CellP.Gradients.InternalEnergy[p][1]*CellP.Gradients.InternalEnergy[p][1] +
                                         CellP.Gradients.InternalEnergy[p][2]*CellP.Gradients.InternalEnergy[p][2]) / CellP.InternalEnergy[p];
                double L_cond = DMAX(L_particle , 1./(L_cond_inv + 1./L_particle)) * All.cf_atime;
                double dt_conduction = dt_prefac_diffusion * L_cond*L_cond / (MIN_REAL_NUMBER + CellP.Kappa_Conduction[p]);
                // since we use CONDUCTIVITIES, not DIFFUSIVITIES, we need to add a power of density to get the right units //
                dt_conduction *= CellP.Density[p] * All.cf_a3inv;
#ifdef SUPER_TIMESTEP_DIFFUSION
...
#ifdef MHD_NON_IDEAL
            {
                ...
                for(int k=0;k<3;k++)
                {
                    b_grad += CellP.Gradients.B[p][k].norm_sq();
                ...
                double diff_coeff = fabs(CellP.Eta_MHD_OhmicResistivity_Coeff[p]) + fabs(CellP.Eta_MHD_HallEffect_Coeff[p]) + fabs(CellP.Eta_MHD_AmbiPolarDiffusion_Coeff[p]);
            ...
            for(k_CRegy=0;k_CRegy<N_CR_PARTICLE_BINS;k_CRegy++)
            {
                if(Get_Gas_CosmicRayPressure(p,k_CRegy) > 1.0e-20)
                {
                    ...
                    double kappa_cr_eff = fabs(CellP.CosmicRayDiffusionCoeff[p][k_CRegy]);
                    ...
                    if((CellP.CosmicRayEnergy[p][k_CRegy]==0)||(CellP.DtCosmicRayEnergy[p][k_CRegy]==0))
                    {
                        dt_conduction = 10. * dt;
                    } else {
                        double delta_cr = dt_conduction*fabs(CellP.DtCosmicRayEnergy[p][k_CRegy]);
                        ...
                        if((dL_cr > thres_dL) || (delta_cr < thres_egy*CellP.CosmicRayEnergy[p][k_CRegy]))
                        {
                            double dt_weak = DMIN(L_cr_weak*coeff_inv , (delta_cr + 1.e-4*CellP.CosmicRayEnergy[p][k_CRegy])/fabs(CellP.DtCosmicRayEnergy[p][k_CRegy]));
                            if((dL_cr > thres_dL+1.) && (delta_cr < 0.1*thres_egy*CellP.CosmicRayEnergy[p][k_CRegy])) {dt_conduction = dt_weak; explicit_timestep_on = 0;}
                        }
                    }
#endif
#ifdef SUPER_TIMESTEP_DIFFUSION
                    ...
                    {
                        if(CellP.CosmicRayEnergy[p][k_CRegy] > 0)
                        {
                            double cr_speed = cr_m1_speed;
                            //double crv=0; int k; for(k=0;k<3;k++) {crv+=CellP.CosmicRayFlux[p][k_CRegy][k]*CellP.CosmicRayFlux[p][k_CRegy][k];} if(crv > 0) {crv = sqrt(crv) / CellP.CosmicRayEnergy[p][k_CRegy];}
                            cr_speed = DMAX( DMIN(cr_m1_speed , CellP.MaxSignalVel[p]) , DMIN(cr_m1_speed , kappa_cr_eff/(Get_Particle_Size(p)*All.cf_atime))); // default to min of free-streaming/diffusion speed
...
#if defined(RADTRANSFER)
            {
                ...
                int kf; for(kf=0;kf<N_RT_FREQ_BINS;kf++)
                {
                    ...
                    double gradETmag = CellP.Gradients.Rad_E_gamma_ET[p][kf].norm_sq();
                    double L_ETgrad_inv = sqrt(gradETmag) / (1.e-37 + CellP.Rad_E_gamma[p][kf] * CellP.Density[p]/P.Mass[p]);
                    ...
                    double dt_rt_work = All.CourantFac * DMIN( L_RT_diffusion / csnd , L_particle*All.cf_atime / ((2./3.)*sqrt(CellP.Rad_E_gamma[p][kf]/P.Mass[p])) ); /* time-step related to radiation work, radiation soundspeed, relevant in strongly-coupled limit */
#ifdef RT_FLUXLIMITER /* if we are flux-limited, we can account for the flux limiter making the timestep advective */
                    if(dt_advective > dt_rt_diffusion) {dt_rt_diffusion *= 1. + (1.-CellP.Rad_Flux_Limiter[p][kf]) * DMAX(0,(dt_advective/dt_rt_diffusion-1.));}
                    ...
                    dt_rt_work /= MIN_REAL_NUMBER + CellP.Rad_Flux_Limiter[p][kf];
                    if((CellP.Rad_Flux_Limiter[p][kf] <= 0)||(dt_rt_diffusion<=0)) {dt_rt_diffusion = 1.e9 * dt;}
#endif
                    if((CellP.Rad_E_gamma[p][kf] <= MIN_REAL_NUMBER) || (CellP.Rad_E_gamma_Pred[p][kf] <= MIN_REAL_NUMBER)) {dt_rt_diffusion = dt_advective;} /* if the radiation is totally negligible, just use an advective timestep instead */
                    ...
                    double gradErad = CellP.Gradients.Rad_E_gamma_ET[p][kf].norm_sq();
                    double radacc = return_flux_limiter(p,kf) * (sqrt(gradErad) / CellP.Density[p]) / All.cf_atime; // radiation acceleration for a timestep criterion
                    if(gradErad > 0 && radacc > 0)
                    {
                        double dt_radacc = sqrt(2 * All.ErrTolIntAccuracy * All.cf_atime * KERNEL_CORE_SIZE * DMAX(ForceSoftening_KernelRadius(p), P.KernelRadius[p]) / radacc);
                ...
                if((CellP.MaxSignalVel[p] > 0.5*C_LIGHT_CODE_REDUCED(p)) || (P.ID[p] == All.SpawnedWindCellID && P.Type[p] == 0)) {dt_courant *= 0.5;}; // be more careful if this is a jet cell or there are transluminal velocities
#endif
#endif                
                ...
                int kf; for(kf=0;kf<N_RT_FREQ_BINS;kf++)
                {
                    ...
                    if((CellP.Rad_E_gamma[p][kf] <= MIN_REAL_NUMBER) || (CellP.Rad_E_gamma_Pred[p][kf] <= MIN_REAL_NUMBER) || (CellP.Rad_E_gamma[p][kf] < 1.e-5*P.Mass[p]*CellP.InternalEnergy[p])) {dt_rt_diffusion = 1.e10 * dt;} /* ignore particles where the radiation energy density is negligible */
                ...
                double ne_cgs = (CellP.Density[p] * All.cf_a3inv * UNIT_DENSITY_IN_NHCGS), dt_recombination = All.CourantFac * (3.3e12/ne_cgs) / UNIT_TIME_IN_CGS;
                double dt_change = 1.e10*dt; if((CellP.Rad_E_gamma[p][RT_FREQ_BIN_H0] > 0)&&(fabs(CellP.Dt_Rad_E_gamma[p][RT_FREQ_BIN_H0])>0)) {dt_change = CellP.Rad_E_gamma[p][RT_FREQ_BIN_H0] / fabs(CellP.Dt_Rad_E_gamma[p][RT_FREQ_BIN_H0]);}
...
#ifdef VISCOSITY
            {
                ...
                v_mag += P.Vel[p].norm_sq();
                double dv_mag_all = 0.0;
                for(kv1=0;kv1<3;kv1++)
                {
                    double dvmag_tmp = CellP.Gradients.Velocity[p][kv1].norm_sq();
                    dv_mag += dvmag_tmp /DMAX(P.Vel[p][kv1]*P.Vel[p][kv1],0.01*v_mag);
                ...
                double visc_coeff = sqrt(CellP.Eta_ShearViscosity[p]*CellP.Eta_ShearViscosity[p] + CellP.Zeta_BulkViscosity[p]*CellP.Zeta_BulkViscosity[p]);
                double dt_viscosity = 0.25 * L_visc*L_visc / (1.0e-33 + visc_coeff) * CellP.Density[p] * All.cf_a3inv;
...
#if defined(GRAIN_BACKREACTION)
            if(6.*P.Grain_AccelTimeMin[p] < dt) {dt = 6.*P.Grain_AccelTimeMin[p];}
#endif
...
#ifdef TURB_DIFFUSION
            {
                ...
                {
                    double dt_tdiff = L_tdiff*L_tdiff / (1.0e-33 + CellP.TD_DiffCoeff[p]); // here, we use DIFFUSIVITIES, so there is no extra density power in the equation //
            ...
            double fac_magnetic_pressure = 1. / All.cf_atime;
            double phi_b_units = Get_Gas_PhiField(p) / ( All.cf_atime * CellP.MaxSignalVel[p]);
            double vsig1 =  sqrt( Get_Gas_effective_soundspeed_i(p)*Get_Gas_effective_soundspeed_i(p) +
                    fac_magnetic_pressure * (Get_Gas_BField(p,0)*Get_Gas_BField(p,0) +
                                             ...
                                             phi_b_units*phi_b_units) / CellP.Density[p] );
            ...
            double divVel = P.Particle_DivVel[p];
            if(divVel != 0)
            {
            ...
            /* now use the timestep information above to limit the super-stepping timestep */
            {
                ...
                if(CellP.Super_Timestep_j[p] == 0) {CellP.Super_Timestep_Dt_Explicit[p] = dt_superstep_explicit;} // reset dt_explicit //
                double j_p_super = (double)(CellP.Super_Timestep_j[p] + 1);
                double dt_superstep = CellP.Super_Timestep_Dt_Explicit[p] / ((nu_substeps+1) + (nu_substeps-1) * cos(M_PI * (2*j_p_super - 1) / (2*(double)N_substeps)));

                double dt_touse = dt_superstep;
                if((dt <= dt_superstep)||(CellP.Super_Timestep_j[p] > 0))
                {
                    /* if(dt <= dt_superstep): other constraints beat our super-step, so it doesn't matter: iterate */
                    /* if(CellP.Super_Timestep_j[p] > 0): don't break mid-cycle, so iterate */
                    CellP.Super_Timestep_j[p]++; if(CellP.Super_Timestep_j[p]>=N_substeps) {CellP.Super_Timestep_j[p]=0;} /*!< increment substep 'j' and loop if it cycles fully */
                } else {
                    ...
                    if(dt_superstep > 1.5*dt_allowed)
                    {
                        ...
                        CellP.Super_Timestep_j[p]++; if(CellP.Super_Timestep_j[p]>=N_substeps) {CellP.Super_Timestep_j[p]=0;}
                    }
                }
        ...
        } // closes if(P.Type[p] == 0) [gas particle check] //


#if defined(DM_SIDM)
    /* Reduce time-step if this particle got interaction probabilities > 0.2 during the last time-step */
    if((1 << P.Type[p]) & (DM_SIDM))
    {
        if(P.dtime_sidm[p] > 0) {if(P.dtime_sidm[p] < dt) {dt = P.dtime_sidm[p];}}
        if(dt > 0)
        {
            double p_target = 0.2; // desired maximum probability per timestep
            double vsig_fac = P.AGS_vsig[p]*All.cf_atime/sqrt(3.);
            Vec3<double> dV = {vsig_fac, vsig_fac, vsig_fac}; // convert signal vel to velocity dispersion for estimating rates
#ifdef GRAIN_COLLISIONS
            double p_dt = prob_of_grain_interaction(return_grain_cross_section_per_unit_mass(p),P.Mass[p],0.,P.AGS_KernelRadius[p],dV.data_ptr(),dt,p); // probability of interacting with another grain super-particle well within kernel, assuming same mass, H, and V~signalvel, for current timestep dt
#else
            double p_dt = prob_of_interaction(P.Mass[p],0.,P.AGS_KernelRadius[p],dV.data_ptr(),dt); // probability of interacting with another DM particle well within kernel, assuming same mass, H, and V~signalvel, for current timestep dt
#endif
    ...
    if(((P.Type[p] == 4)||((All.ComovingIntegrationOn==0)&&((P.Type[p] == 2)||(P.Type[p]==3))))&&(P.Mass[p]>0))
    {
        ...
        double mcorr = 1.e-4 * (P.Mass[p]*UNIT_MASS_IN_SOLAR) / 0.1; // expectation of ()/X SNe per timestep -- here 0.1
...
*/
#else
        double mcorr = 1.e-5 * (P.Mass[p]*UNIT_MASS_IN_SOLAR);
    ...
    if(P.Type[p] == 5)
    {
      ...
      double dt_accr = All.MaxSizeTimestep;
#endif
        if(P.Sink_Mdot[p] > 0 && P.Sink_Mass[p] > 0 && All.Time > All.TimeBegin)
        {
            ...
            dt_accr = 0.001 * DMAX(P.Sink_Mass[p], All.MaxMassForParticleSplit) / P.Sink_Mdot[p];
#if defined(SINK_WIND_KICK)
            dt_accr *= DMAX(0.1, All.Sink_accreted_fraction);
#endif
#else
            dt_accr = 0.1 * DMIN(P.Sink_Mass[p], All.MaxMassForParticleSplit) / P.Sink_Mdot[p];
#endif
#ifdef SINGLE_STAR_FB_JETS	    
            dt_accr = DMIN(dt_accr, target_mass_for_wind_spawning(p) / P.Sink_Mdot[p]); 
#endif
        } // if(P.Sink_Mdot[p] > 0 && P.Sink_Mass[p] > 0)
        ...
        double dt_ngbs = 4.1 * GET_PHYSICAL_TIMESTEP_FROM_TIMEBIN(P.Sink_TimeBinGasNeighbor[p],p); /* standard wakeup-type threshold: use this by default here, unless dynamical interaction important (e.g. back-rx term from oscillation of sink c-o-m, which is important for single-sink sims */
        if(dt > dt_ngbs && dt_ngbs > 0) {dt = 1.01 * dt_ngbs; }

#if defined(SINGLE_STAR_TIMESTEPPING)
	    if(P.DensityAroundParticle[p] > 0)
	    {
            double eps = DMAX( KERNEL_CORE_SIZE*ForceSoftening_KernelRadius(p), P.Sink_dr_to_NearestGasNeighbor[p]);
#ifdef SINK_GRAVCAPTURE_FIXEDSINKRADIUS
            eps = DMAX(eps, P.SinkRadius[p]);
#endif
            if(eps < MAX_REAL_NUMBER) {eps = DMAX(Get_Particle_Size(p), eps);} else {eps = Get_Particle_Size(p);}
#if (ADAPTIVE_GRAVSOFT_FORALL & 32)
            eps = DMAX(eps, KERNEL_CORE_SIZE*P.AGS_KernelRadius[p]);
#endif
            double dt_ff = sqrt(2*All.ErrTolIntAccuracy * pow(eps*All.cf_atime,3) / (All.G * P.Mass[p])); // fraction of the freefall time of the nearest gas particle from rest
            if(dt > dt_ff && dt_ff > 0) {dt = 1.01 * dt_ff;}

            double L_particle = Get_Particle_Size(p);
            double vsig = P.Sink_SurroundingGasVel[p];
#if defined(SINGLE_STAR_FB_TIMESTEPLIMIT) && !defined(NOGRAVITY)
            vsig += P.MaxFeedbackVel[p];
#endif                        
        ...
        if ( (P.ProtoStellarStage[p] == 6) && ( (P.Sink_Mass[p] > 0) || (P.unspawned_wind_mass[p] > 0) ) ) { //Star going supernova, still has mass to eject
            double eps = DMIN(KERNEL_CORE_SIZE*ForceSoftening_KernelRadius(p), P.KernelRadius[p]);
#ifdef SINK_GRAVCAPTURE_FIXEDSINKRADIUS
            eps = DMAX(eps, P.SinkRadius[p]);
#endif
    ...
    if((P.Type[p]==5) || (P.Type[p]==0 && P.ID[p]==All.SpawnedWindCellID && CellP.IniDen[p]<0)) {if(dt>All.Sink_spawn_injectionradius/All.Sink_outflow_velocity && All.Sink_spawn_injectionradius>0 && All.Sink_outflow_velocity>0) {dt=All.Sink_spawn_injectionradius/All.Sink_outflow_velocity;}}
#endif
    ...
    if((dt < All.MinSizeTimestep)||(((integertime) (dt / All.Timebase_interval)) <= 1))
    {
        ...
        double agrav_pm=0, agrav = P.GravAccel[p].norm() * All.cf_a2inv;
#ifdef PMGRID
        agrav_pm = P.GravPM[p].norm() * All.cf_a2inv;
#endif
        if(P.Type[p] == 0)
        {
            double aturb=0, arad=0, ahydro = CellP.HydroAccel[p].norm();
#ifdef TURB_DRIVING
            aturb = CellP.TurbAccel[p].norm();
#endif
#ifdef RT_RAD_PRESSURE_OUTPUT
            arad = CellP.Rad_Accel[p].norm();
#endif
            PRINT_WARNING("\n Cell-ID=%llu  dt_desired=%g dt_Courant=%g dt_Accel=%g\n accel_tot=%g accel_gravTree=%g accel_gravPM=%g accel_hydro=%g accel_rad=%g accel_turb=%g Pos_xyz=(%g|%g|%g) Vel_xyz=(%g|%g|%g)\n KernelRadius=%g Density=%g InternalEnergy=%g dtInternalEnergy=%g divV=%g Pressure=%g Cs_Eff=%g vAlfven=%g f_ion=%g\n csnd_for_signalspeed=%g eps_forcesoftening=%g mass=%g type=%d condition_number=%g Nngb=%g\n NVT=%.17g/%.17g/%.17g %.17g/%.17g/%.17g %.17g/%.17g/%.17g\n",
                          (unsigned long long) P.ID[p], dt, dt_courant*All.cf_hubble_a, sqrt(2*All.ErrTolIntAccuracy*All.cf_atime*ForceSoftening_KernelRadius(p) / ac)*All.cf_hubble_a,
                          ac, agrav, agrav_pm, ahydro, arad, aturb, P.Pos[p][0], P.Pos[p][1], P.Pos[p][2], P.Vel[p][0]/All.cf_atime, P.Vel[p][1]/All.cf_atime, P.Vel[p][2]/All.cf_atime,
                          P.KernelRadius[p]*All.cf_atime, CellP.Density[p]*All.cf_a3inv, CellP.InternalEnergy[p], CellP.DtInternalEnergy[p], P.Particle_DivVel[p]*All.cf_a2inv,
                          CellP.Pressure[p]*All.cf_a3inv, Get_Gas_effective_soundspeed_i(p), Get_Gas_Alfven_speed_i(p), Get_Gas_Ionized_Fraction(p),
                          csnd, ForceSoftening_KernelRadius(p)*All.cf_atime, P.Mass[p], P.Type[p], CellP.ConditionNumber[p], P.NumNgb[p],
                          CellP.NV_T[p][0][0],CellP.NV_T[p][0][1],CellP.NV_T[p][0][2],CellP.NV_T[p][1][0],CellP.NV_T[p][1][1],CellP.NV_T[p][1][2],CellP.NV_T[p][2][0],CellP.NV_T[p][2][1],CellP.NV_T[p][2][2]);
        }
        else // if(P.Type[p] == 0)
        {
            PRINT_WARNING("Part-ID=%llu  dt_desired=%g dt_Accel=%g\n accel_tot=%g accel_gravTree=%g accel_gravPM=%g  mass=%g pos_xyz=(%g|%g|%g) vel_xyz=(%g|%g|%g) soft=%g type=%d\n",
                          (unsigned long long) P.ID[p], dt, sqrt(2*All.ErrTolIntAccuracy*All.cf_atime*ForceSoftening_KernelRadius(p) / ac)*All.cf_hubble_a,
                          ac, agrav, agrav_pm, P.Mass[p], P.Pos[p][0], P.Pos[p][1], P.Pos[p][2], P.Vel[p][0], P.Vel[p][1], P.Vel[p][2], ForceSoftening_KernelRadius(p), P.Type[p]);
        }
        fflush(stdout); fprintf(stderr, "\n @ fflush \n");
#ifdef STOP_WHEN_BELOW_MINTIMESTEP
        if(P.Mass[p] > 0) {endrun(888);}
#endif
    ...
    if(!(ti_step > 0 && ti_step < TIMEBASE))
    {
        printf("\nError: A timestep of size zero was assigned on the integer timeline. Code must stop.\n"
               ...
               ThisTask, (unsigned long long) P.ID[p], dt, dt_courant, dt_divv, dt_displacement,
               All.Timebase_interval, (long long) ti_step, ac, P.Pos[p][0], P.Pos[p][1], P.Pos[p][2], P.GravAccel[p][0], P.GravAccel[p][1], P.GravAccel[p][2]);
#ifdef PMGRID
        printf("pm_force=(%g|%g|%g)\n", P.GravPM[p][0], P.GravPM[p][1], P.GravPM[p][2]);
#endif
...
void find_dt_displacement_constraint(double hfac /*!<  should be  a^2*H(a)  */ )
{
    ...
    if(All.ComovingIntegrationOn)
    {
        ...
        for(i = 0; i < NumPart; i++)
        {
            if(P.Mass[i] > 0)
            {
                double v2 = P.Vel[i].norm_sq();
                if(v2 > 0 && isfinite(v2)) {
                    count[P.Type[i]]++;
                    if(P.Type[i] == 0) {v[P.Type[i]] += P.Mass[i] * v2;} else {v[P.Type[i]] += v2;} /* for gas use a weighted average to deal with extreme cell-mass difference situations */
                    if(mim[P.Type[i]] > P.Mass[i]) {mim[P.Type[i]] = P.Mass[i];}
                    mnm[P.Type[i]] += P.Mass[i];
                }
            }
        }
...
void process_wake_ups(void)
{
	...
	for(i = 0; i < NumPart; i++)
	{
	    if(!P.wakeup[i]) {continue;}
#if !defined(AGS_KERNELRADIUS_CALCULATION_IS_ACTIVE)
	    if(P.Type[i] != 0) {continue;} // only gas particles can be awakened
#endif
	    ...
	    if(bin != binold)
	    {
		integertime dt_0 = GET_INTEGERTIME_FROM_TIMEBIN(P.TimeBin[i]);
		integertime tstart = P.Ti_begstep[i] + dt_0;
		integertime t_2 = P.Ti_current[i];
		if(t_2 > tstart) {tstart = t_2;}
		integertime tend = All.Ti_Current;

		TimeBinCount[binold]--;
		if(P.Type[i] == 0) {TimeBinCountGas[binold]--;}
		...
		P.Ti_begstep[i] = All.Ti_Current;
		P.dt_step[i] = GET_INTEGERTIME_FROM_TIMEBIN(bin);
		if(P.Ti_current[i] < All.Ti_Current) {P.Ti_current[i]=All.Ti_Current;}
	    }
	}
    }
