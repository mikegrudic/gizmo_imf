/* SoA (Structure of Arrays) version of particle_data.h
 * Auto-generated by convert_to_soa.py
 * Original struct fields become per-field arrays.
 * Access pattern: P[i].Field -> P.Field[i]
 */
...
void copy_particle_between_P(particle_soa& dest, int di, particle_soa& src, int si);
void move_particles_P(particle_soa& s, int dest, int src, int count);
void MPI_Sendrecv_P_fields(particle_soa& src, long src_offset, long count,
                           particle_soa& dest, long dest_offset, long recv_count,
                           int target, int tag);
...
struct particle_soa
{
    ...
    Vec3<MyDouble> *Pos; /*!< particle position at its current time */
...
#define AREA_WEIGHTED_SUM_ELEMENTS 12 /* number of weights needed for full momentum-and-energy conserving system */
    ...
    int *SubNr;
    int *DM_NumNgb;
    unsigned short *targettask;
    unsigned short *origintask2;
    int *origintask;
    int *submark;
    int *origindex;
...
extern struct particle_soa P; /* SoA data on local processor */
extern struct particle_soa DomainPartBuf; /* buffer for domain decomposition */
