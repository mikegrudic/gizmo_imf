...
{
    if(!((1 << P.Type[i]) & (RT_SOURCES))) {return 0;}; // boolean test of whether i is a source or not - end if not a valid source particle
    if(P.Mass[i] <= 0 || !isfinite(P.Mass[i])) {return 0;} // reject invalid particles scheduled for deletion
    ...
    {   // here we have type=5, treat as a sink particle/single-star
        if(P.ProtoStellarStage[i] != 7) {active_check += rt_get_lum_band_singlestar(i,mode,lum); /* stars and protostars */} else {active_check += rt_get_lum_band_agn(i,mode,lum); /* relics */}
    ...
    if(P.Type[i]==4)
    {
        SET_ACTIVE_RT_CHECK(); double l_ion=All.IonizingLuminosityPerSolarMass_cgs * (P.Mass[i] * UNIT_MASS_IN_SOLAR) / UNIT_LUM_IN_CGS; // flux from star particles according to mass
    ...
    if(P.Type[i] == 4) // set this to whichever type you want to use for the specific sources in this band
    {
...
#endif
        lum[RT_FREQ_BIN_GENERIC_USER_FREQ] = (P.Mass[i]/1.) * All.Vertical_Grain_Accel * C_LIGHT_CODE * ((All.Grain_Internal_Density/UNIT_DENSITY_IN_CGS)*(All.Grain_Size_Max/UNIT_LENGTH_IN_CGS)) * A_base / (0.75*All.Grain_Q_at_MaxGrainSize); // special behavior for particular test of stratified boxes compared to explicit dust opacities
#endif
    }
#endif
...
#ifdef RADTRANSFER
    if(P.Type[i] == 0) /* generic sub routines for gas as a source term, should go at the very end of this routine */
    {
        ...
        int k; for(k=0;k<N_RT_FREQ_BINS;k++) {lum[k] += CellP.Rad_Je[i][k];}
    }
#endif
...
double rt_kappa(int i, int k_freq)
{
    ...
    return CellP.Interpolated_Opacity[i][k_freq] + 1.e-3 * All.Dust_to_Gas_Mass_Ratio*0.75*All.Grain_Q_at_MaxGrainSize/((All.Grain_Internal_Density/UNIT_DENSITY_IN_CGS)*(All.Grain_Size_Max/UNIT_LENGTH_IN_CGS)); /* enforce minimum; note kappa in code units here so need to convert appropriately */
#endif
    return MIN_REAL_NUMBER + CellP.Interpolated_Opacity[i][k_freq]; /* this is calculated in a different routine, just return it now */
#endif
    ...
    double kappa = nH_over_Density * (CellP.HI[i] + MIN_REAL_NUMBER) * rt_ion_sigma_HI[k_freq]; // note this is designed for specific applications: does not include dust, or free-free, or free-electron scattering contributions here, all of which can be important.
#if defined(RT_CHEM_PHOTOION_HE) && defined(RT_PHOTOION_MULTIFREQUENCY)
    kappa += nH_over_Density * ((CellP.HeI[i] + MIN_REAL_NUMBER) * rt_ion_sigma_HeI[k_freq] + (CellP.HeII[i] + MIN_REAL_NUMBER) * rt_ion_sigma_HeII[k_freq]);
...
#ifdef METALS
    if(i>=0) {Zfac = P.Metallicity[i][0]/All.SolarAbundances[0];}
#endif
#ifdef COOLING
    if(i>=0) {kappa_HHe=0.02 + 0.35*CellP.Ne[i];}
#endif
    ...
    {
        double T_eff=0.59*(GAMMA(i)-1.)*U_TO_TEMP_UNITS*CellP.InternalEnergyPred[i], rho=CellP.Density[i]*All.cf_a3inv*UNIT_DENSITY_IN_CGS; // we're assuming fully-ionized gas with a simple equation-of-state here, nothing fancy, to get the temperature //
    ...
    {
        if(isnan(CellP.Dust_Temperature[i])) {PRINT_WARNING("\n NaN dust temperature for cell-ID=%llu  \n", (unsigned long long) P.ID[i]); CellP.Dust_Temperature[i] = 1.e4;}
        if(isnan(CellP.Radiation_Temperature[i])) {PRINT_WARNING("\n NaN gas temperature for cell-ID=%llu  \n", (unsigned long long) P.ID[i]);}
        if(CellP.Dust_Temperature[i]<=T_min) {CellP.Dust_Temperature[i]=T_min;} // reset baseline
        if(CellP.Radiation_Temperature[i]<=T_min) {CellP.Radiation_Temperature[i]=T_min;} // reset baseline
        double T_dust_em = CellP.Dust_Temperature[i]; // dust temperature in K //
        double Trad = CellP.Radiation_Temperature[i]; // radiation temperature in K //
        if((Trad <= 0) || (T_dust_em<=0)) {PRINT_WARNING("\n Cell-ID=%llu  has T_rad=%g and T_dust=%g\n", (unsigned long long) P.ID[i], Trad, T_dust_em);}
...
double rt_absorb_frac_albedo(int i, int k_freq)
{
    ...
    if(k_freq==RT_FREQ_BIN_INFRARED) {return rt_kappa_adaptive_IR_band(i,CellP.Dust_Temperature[i],CellP.Radiation_Temperature[i],-1,0) / (MIN_REAL_NUMBER + rt_kappa_adaptive_IR_band(i,CellP.Dust_Temperature[i],CellP.Radiation_Temperature[i],0,0));}
#endif
#endif
    ...
    if(k_freq==RT_FREQ_BIN_FREEFREE)
    {
        double T_eff=0.59*(GAMMA(i)-1.)*U_TO_TEMP_UNITS*CellP.InternalEnergyPred[i], rho=CellP.Density[i]*All.cf_a3inv*UNIT_DENSITY_IN_CGS, kappa_abs = 1.e30*rho*pow(T_eff,-3.5);
...
{
    if(!((P.Type[i] == 4) || ((All.ComovingIntegrationOn==0)&&((P.Type[i]==2)||(P.Type[i]==3))))) {return 0;} // only star-type particles act in this subroutine //
    if(P.Mass[i] <= 0 || !isfinite(P.Mass[i])) {return 0;}
    int active_check = 0; // default to inactive //
#if defined(GALSF) /* basically none of these modules make sense without the GALSF module active */
    double star_age = evaluate_stellar_age_Gyr(i), m_sol = P.Mass[i] * UNIT_MASS_IN_SOLAR;
    ...
    double sigma_eff = evaluate_NH_from_GradRho(P.GradRho[i],P.KernelRadius[i],P.DensityAroundParticle[i],P.NumNgb[i],0,i); if((sigma_eff <= 0)||(isnan(sigma_eff))) {sigma_eff=0;} // sigma here is in code units
...
{
    if(P.Type[i] != 5) {return 0;} // only go forward for BH-type particles
    if(P.Mass[i] <= 0 || !isfinite(P.Mass[i])) {return 0;}
    int active_check = 0; // default to inactive //
#if defined(SINK_PARTICLES)
    double l_bol = sink_lum_bol(P.Sink_Mdot[i],P.Mass[i],i); if(l_bol <= 0) {return 0;} // no accretion luminosity -- no point in going further!
...
{
    if(P.Type[i] < 4) {return 0;} // only go forward with star or sink-type particles
    if(P.Mass[i] <= 0 || !isfinite(P.Mass[i])) {return 0;}
...
double c_light_RSL_reductionfactor_local(int i)
{
    ...
    {
        double dp[3]={0},r2=0; for(k=0;k<3;k++) {dp[k] = All.cf_atime*(P.Pos[i][k] - All.SpecialParticle_Position_ForRefinement[j][k]); r2 += dp[k]*dp[k];}
...
double rt_absorption_rate(int i, int k_freq)
{
    ...
    return (C_LIGHT_CODE_REDUCED(i)) * rt_absorb_frac_albedo(i,k_freq) * (rt_kappa(i,k_freq) * CellP.Density[i]*All.cf_a3inv);
}
#endif 
...
{
    return return_flux_limiter(i,k_freq) * C_LIGHT_CODE_REDUCED(i) / (1.e-45 + CellP.Rad_Kappa[i][k_freq] * CellP.Density[i]*All.cf_a3inv);
}
...
void rt_eddington_update_calculation(int j)
{
    ...
    int k_freq, k; double fmag_j, V_j_inv = CellP.Density[j] / P.Mass[j]; Vec3<double> n_flux_j;
    for(k_freq=0;k_freq<N_RT_FREQ_BINS;k_freq++)
    {
        n_flux_j = {};
        Vec3<double> flux_vol = CellP.Rad_Flux[j][k_freq] * V_j_inv;
        fmag_j = flux_vol.norm_sq();
        if(fmag_j <= 0) {fmag_j=0;} else {fmag_j=sqrt(fmag_j); n_flux_j = flux_vol/fmag_j;}
        double f_chifac = fmag_j / (MIN_REAL_NUMBER + C_LIGHT_CODE_REDUCED(j) * CellP.Rad_E_gamma[j][k_freq] * V_j_inv);
        ...
        for(k=0;k<3;k++) for(int k2=k;k2<3;k2++) {CellP.ET[j][k_freq][k][k2] = chifac_n_j * n_flux_j[k]*n_flux_j[k2];}
        CellP.ET[j][k_freq][0][0] += chifac_iso_j; CellP.ET[j][k_freq][1][1] += chifac_iso_j; CellP.ET[j][k_freq][2][2] += chifac_iso_j;
    }
    return;
#endif
#ifdef RT_FLUXLIMITEDDIFFUSION
    /* always assume the isotropic eddington tensor */
    int k_freq; for(k_freq=0;k_freq<N_RT_FREQ_BINS;k_freq++) {CellP.ET[j][k_freq].set_isotropic(1./3.);}
    return;
#endif
...
double return_flux_limiter(int target, int k_freq)
{
    ...
    return CellP.Rad_Flux_Limiter[target][k_freq]; // apply flux-limiter
#endif
    return 1;
}
...
void rt_update_driftkick(int i, double dt_entr, int mode)
{
    ...
    for(kf=0;kf<N_RT_FREQ_BINS;kf++) {CellP.Rad_E_gamma[i][kf]=0; for(k_tmp=0;k_tmp<N_RT_INTENSITY_BINS;k_tmp++) {CellP.Rad_E_gamma[i][kf]+=RT_INTENSITY_BINS_DOMEGA*CellP.Rad_Intensity[i][kf][k_tmp];}}
    ...
    {int j; for(j=0;j<N_RT_FREQ_BINS;j++) {Rad_E_gamma_tot += CellP.Rad_E_gamma[i][j];}}
    double a_rad_inverse=C_LIGHT_CGS/(4.*5.67e-5), vol_inv_phys=(CellP.Density[i]*All.cf_a3inv/P.Mass[i]), u_gamma = Rad_E_gamma_tot * vol_inv_phys * UNIT_PRESSURE_IN_CGS; // photon energy density in CGS //
    ...
    CellP.Dust_Temperature[i] = sqrt(sqrt(Dust_Temperature_4)); // just set this to the local radiation equilibrium temperature
#endif
    double T_min = get_min_allowed_dustIRrad_temperature();
    if(CellP.Dust_Temperature[i] <= T_min) {CellP.Dust_Temperature[i] = T_min;} // dust temperature shouldn't be below CMB
    ...
    double T_gas = CellP.Dust_Temperature[i];
#ifdef COOLING
...
#endif
        {
...
#if defined(RT_EVOLVE_INTENSITIES)
            if(mode==0) {e0 = RT_INTENSITY_BINS_DOMEGA*CellP.Rad_Intensity[i][kf][k_angle];} else {e0 = RT_INTENSITY_BINS_DOMEGA*CellP.Rad_Intensity_Pred[i][kf][k_angle];}
            dt_e_gamma_band = RT_INTENSITY_BINS_DOMEGA*CellP.Dt_Rad_Intensity[i][kf][k_angle];
#else
            if(mode==0) {e0 = CellP.Rad_E_gamma[i][kf];} else {e0 = CellP.Rad_E_gamma_Pred[i][kf];}
            dt_e_gamma_band = CellP.Dt_Rad_E_gamma[i][kf];
#endif
#ifdef RT_COMOVING
            double ET_dotdot_GradVcom = CellP.ET[i][kf].double_contract(CellP.Gradients.Velocity[i]);
...
#endif
            total_de_dt = CellP.Rad_Je[i][kf] + dt_e_gamma_band;
            ...
            if(kf == RT_FREQ_BIN_INFRARED)
            {
                if((mode==0) && (dt_e_gamma_band!=0) && (dt_entr>0)) // only update temperatures on kick operations //
                {
                    ...
                    double dTE_fac = CellP.Dt_Rad_E_gamma_T_weighted_IR[i] * dt_entr; // T-weighted change from advection
                    double dE_abs = -e0 * (1. - exp(a0_abs*dt_entr)); // change in energy from absorption
                    double T_max = DMAX(CellP.Radiation_Temperature[i] , dE_fac / dTE_fac); 
                    ...
                    CellP.Radiation_Temperature[i] = (e0 + dE_fac) / (MIN_REAL_NUMBER + DMAX(0., e0 / CellP.Radiation_Temperature[i] + dTE_fac));
                    CellP.Radiation_Temperature[i] = DMIN(CellP.Radiation_Temperature[i], T_max);
                ...
                CellP.Dust_Temperature[i] = rt_eqm_dust_temp(i, T_gas, total_absorption_rate * vol_inv_phys * C_LIGHT_CODE / C_LIGHT_CODE_REDUCED(i));
                if(CellP.Dust_Temperature[i] < T_min) {CellP.Dust_Temperature[i] = T_min;}
                double Tdust_eff = CellP.Dust_Temperature[i], Trad_eff = CellP.Radiation_Temperature[i];
                ...
                double total_emission_rate = total_absorption_rate * (1.-IRBand_opacity_fraction_from_gas_absorption) + CellP.Rad_Je[i][kf]; /* we will re-radiate this much because the component due to gas-dust coupling is accounted for in the cooling loop */
                total_de_dt = E_abs_tot_toIR + CellP.Rad_Je[i][kf] + dt_e_gamma_band;
                if((mode==0) && (Tdust_eff <= MAX_DUST_TEMP)) // only update temperatures on kick operations and Tdust is meaningful //
                {
                    ...
                    double etot_em_gas = e_absorbed * IRBand_opacity_fraction_from_gas_absorption + CellP.Rad_Je[i][kf]*dt_entr;
                    ...
                    CellP.Radiation_Temperature[i] = DMIN( Trad_new , T_max );
                }
                if(CellP.Radiation_Temperature[i] < T_min) {CellP.Radiation_Temperature[i] = T_min;} // radiation temperature shouldn't be below CMB
            }
#endif
            ...
            // for OTVET/FLD methods, need to apply radiation pressure term here so can limit this b/c just based on a gradient which is not flux-limited [as in hydro operators] //
            {
                ...
                double Sigma_particle = P.Mass[i] / (M_PI*L_particle*L_particle), abs_per_kappa_dt = C_LIGHT_CODE_REDUCED(i) * (CellP.Density[i]*All.cf_a3inv) * dt_entr; // effective surface density through particle & fractional absorption over timestep
                double f_kappa_abs = rt_absorb_frac_albedo(i,kf); // get albedo, we'll need this below
                double slabfac_rp=1; if(check_if_absorbed_photons_can_be_reemitted_into_same_band(kf)==0) {slabfac_rp=slab_averaging_function(f_kappa_abs*CellP.Rad_Kappa[i][kf]*Sigma_particle) * slab_averaging_function(f_kappa_abs*CellP.Rad_Kappa[i][kf]*abs_per_kappa_dt);} // reduction factor for absorption over dt
                int kx; for(kx=0;kx<3;kx++)
                {
                    radacc[kx] = -dt_entr * slabfac_rp * return_flux_limiter(i,kf) * (CellP.Gradients.Rad_E_gamma_ET[i][kf][kx] / CellP.Density[i]) / All.cf_atime; // naive radiation-pressure calc for FLD methods [physical units]
                    rmag += radacc[kx]*radacc[kx]; // compute magnitude
                    if(mode==0) {vel_i[kx]=(C_LIGHT_CODE_REDUCED(i)/C_LIGHT_CODE)*P.Vel[i][kx]/All.cf_atime;} else {vel_i[kx]=(C_LIGHT_CODE_REDUCED(i)/C_LIGHT_CODE)*CellP.VelPred[i][kx]/All.cf_atime;} // [for comoving] note this is the 'effective' u appearing in the RHD equations for an RSOL, care needed with these factors!
                }
                if(rmag > 0)
                {
                    rmag = sqrt(rmag); for(kx=0;kx<3;kx++) {radacc[kx] /= rmag;} // normalize
                    double rmag_max = de_abs / (P.Mass[i] * C_LIGHT_CODE_REDUCED(i) * (MIN_REAL_NUMBER + f_kappa_abs)); // limit magnitude to absorbed photon momentum
                    ...
                    for(kx=0;kx<3;kx++)
                    {
                        ...
                        work_band += vel_i[kx] * radacc_eff * P.Mass[i]; // PdV work done by photons [absorbed ones are fully-destroyed, so their loss of energy and momentum is already accounted for by their deletion in this limit //
                        if(mode==0) {double dv_rt=radacc_eff*All.cf_atime; P.Vel[i][kx]+=dv_rt; P.dp[i][kx]+=dv_rt*P.Mass[i];} else {CellP.VelPred[i][kx] += radacc_eff * All.cf_atime;}
                    }
                    ...
                    if(mode==0) {CellP.InternalEnergy[i] += d_egy_int;} else {CellP.InternalEnergyPred[i] += d_egy_int;}
#if defined(RT_EVOLVE_INTENSITIES)
                    {int k_q; for(k_q=0;k_q<N_RT_INTENSITY_BINS;k_q++) {if(mode==0) {CellP.Rad_Intensity[i][kf][k_q]+=d_egy_rad/RT_INTENSITY_BINS_DOMEGA;} else {CellP.Rad_Intensity_Pred[i][kf][k_q]+=d_egy_rad/RT_INTENSITY_BINS_DOMEGA;}}}
#else
                    if(mode==0) {CellP.Rad_E_gamma[i][kf]+=d_egy_rad;} else {CellP.Rad_E_gamma_Pred[i][kf]+=d_egy_rad;}
#endif
                }
            }
#endif
            ...
            if(kf==RT_FREQ_BIN_INFRARED) {
                ...
                if(mode==0) {CellP.DtInternalEnergy[i] += (de_abs * IRBand_opacity_fraction_from_gas_absorption) / ((MIN_REAL_NUMBER + dt_entr) * P.Mass[i]);} /* this fraction absorbed by gas goes into a heating rate which can be balanced implicitly in the cooling function later */
#else
            ...
            if(donation_target_bin >= 0) {int k_q; for(k_q=0;k_q<N_RT_INTENSITY_BINS;k_q++) {if(mode==0) {CellP.Rad_Intensity[i][donation_target_bin][k_q] += de_abs/RT_INTENSITY_BINS_DOMEGA;} else {CellP.Rad_Intensity_Pred[i][donation_target_bin][k_q] += de_abs/RT_INTENSITY_BINS_DOMEGA;}}}
            if(ef < 0) {ef=0;}
            if(mode==0) {CellP.Rad_Intensity[i][kf][k_angle] = ef/RT_INTENSITY_BINS_DOMEGA;} else {CellP.Rad_Intensity_Pred[i][kf][k_angle] = ef/RT_INTENSITY_BINS_DOMEGA;}
#else
            if(donation_target_bin >= 0) {if(mode==0) {CellP.Rad_E_gamma[i][donation_target_bin] += de_abs;} else {CellP.Rad_E_gamma_Pred[i][donation_target_bin] += de_abs;}}
            if(ef < 0) {ef=0;}

            if(mode==0) {CellP.Rad_E_gamma[i][kf] = ef;} else {CellP.Rad_E_gamma_Pred[i][kf] = ef;}
#endif

#if defined(RT_EVOLVE_FLUX)
            int k_dir; double f_mag=0, E_rad_forflux=0, vdot_h[3]={0}, vel_i[3]={0}, DeltaFluxEff[3]={0}, rho=CellP.Density[i]*All.cf_a3inv; E_rad_forflux=0.5*(e0+ef); // use energy density averaged over this update for the operation below
            for(k_dir=0;k_dir<3;k_dir++) {if(mode==0) {vel_i[k_dir]=RSOL_CORRECTION_FACTOR_FOR_VELOCITY_TERMS(i)*P.Vel[i][k_dir]/All.cf_atime;} else {vel_i[k_dir]=RSOL_CORRECTION_FACTOR_FOR_VELOCITY_TERMS(i)*CellP.VelPred[i][k_dir]/All.cf_atime;}} // need gas velocity at this time [effective v - note RSOL terms]
            double teqm_inv = CellP.Rad_Kappa[i][kf] * rho * C_LIGHT_CODE_REDUCED(i) + MIN_REAL_NUMBER; // physical code units of 1/time, defines characteristic timescale for coming to equilibrium flux. see notes for CR second-order module for details. //
            {Vec3<double> v_i{vel_i[0],vel_i[1],vel_i[2]}; Vec3<double> vdh = E_rad_forflux * (v_i + CellP.ET[i][kf].matvec(v_i)); vdot_h[0]=vdh[0]; vdot_h[1]=vdh[1]; vdot_h[2]=vdh[2];} // calculate P_rad term and eI term, multiply by radiation energy //
#ifdef RT_COMPGRAD_EDDINGTON_TENSOR // definitely favor this for greater accuracy and reduced noise //
            for(k_dir=0;k_dir<3;k_dir++) {DeltaFluxEff[k_dir] -= (P.Mass[i]/rho) * (C_LIGHT_CODE_REDUCED(i)*C_LIGHT_CODE_REDUCED(i)/teqm_inv) * CellP.Gradients.Rad_E_gamma_ET[i][kf][k_dir]*All.cf_a3inv/All.cf_atime;} // here we compute the nabla.pressure_gradient_tensor term from gradients directly, and use this in the next step after multiplying the flux equation by (tilde[c]^2/dt_eqm_inv) and working in dimensionless time units
#else
            for(k_dir=0;k_dir<3;k_dir++) {DeltaFluxEff[k_dir] += (CellP.Dt_Rad_Flux[i][kf][k_dir]/teqm_inv);} // the nabla.pressure_gradient_tensor is computed in the finite-volume solver, here
#endif
            ...
            if(tau > 0 && isfinite(tau))
            {
                ...
                {
                    double flux_0; if(mode==0) {flux_0 = CellP.Rad_Flux[i][kf][k_dir];} else {flux_0 = CellP.Rad_Flux_Pred[i][kf][k_dir];}
                    ...
                    if(mode==0) {CellP.Rad_Flux[i][kf][k_dir] = flux_f;} else {CellP.Rad_Flux_Pred[i][kf][k_dir] = flux_f;}
                ...
                if(f_mag > 0) // limit the flux according the physical (optically thin) maximum //
                {
                    ...
                    if(f_mag > fmag_max) {for(k_dir=0;k_dir<3;k_dir++) {if(mode==0) {CellP.Rad_Flux[i][kf][k_dir] *= fmag_max/f_mag;} else {CellP.Rad_Flux_Pred[i][kf][k_dir] *= fmag_max/f_mag;}}}
#if defined(GRAIN_RDI_TESTPROBLEM_LIVE_RADIATION_INJECTION)
                    if(P.Pos[i][2]<=0.1) {if(mode==0) {CellP.Rad_Flux[i][kf][0]=CellP.Rad_Flux[i][kf][1]=0; CellP.Rad_Flux[i][kf][2]=fmag_max;} else {CellP.Rad_Flux_Pred[i][kf][0]=CellP.Rad_Flux_Pred[i][kf][1]=0; CellP.Rad_Flux_Pred[i][kf][2]=fmag_max;}}
#endif
                }
            }
#endif
    ...
    {
        int k,k_om; double rho=CellP.Density[i]*All.cf_a3inv, ceff=C_LIGHT_CODE_REDUCED(i), ctrue=C_LIGHT_CODE, teq_inv=CellP.Rad_Kappa[i][kf]*rho*ceff, f_a=rt_absorb_frac_albedo(i,kf), f_s=1.-f_a, b_dot_n[N_RT_INTENSITY_BINS]={0}, beta_2=0.; Vec3<double> beta;
        ...
        for(i_iter=0; i_iter<n_iter; i_iter++)
        {
            ...
            for(k_om=0;k_om<N_RT_INTENSITY_BINS;k_om++) {if(mode==0) {i0[k_om] = RT_INTENSITY_BINS_DOMEGA*CellP.Rad_Intensity[i][kf][k_om];} else {i0[k_om] = RT_INTENSITY_BINS_DOMEGA*CellP.Rad_Intensity_Pred[i][kf][k_om];}}
            for(k=0;k<3;k++) {if(mode==0) {beta[k]=P.Vel[i][k]/(All.cf_atime*ctrue);} else {beta[k]=CellP.VelPred[i][k]/(All.cf_atime*ctrue);}} // need gas velocity at this time; with equations written this way, the 'beta' term is the -true- beta, so we have to use the true SOL
            ...
            Vec3<double> dv_gas = -(flux_f-flux_0)/(ceff*P.Mass[i]); double ke_gas_0=0, ke_gas_f=0, v0g=0, u0=0;
            ...
            double d_ke_gas = 0.5*(ke_gas_f - ke_gas_0)*P.Mass[i], de_gas=-(ctrue/ceff)*(egy_f-egy_0), de_gas_internal=(de_gas-d_ke_gas)/P.Mass[i]; // note ctrue/ceff factor here, accounting for rsol difference in gas heating/cooling rates vs RHD
            if(mode==0) {u0=CellP.InternalEnergy[i];} else {u0=CellP.InternalEnergyPred[i];} // for updating gas internal energy (work terms, after subtracting kinetic energy changes)
            ...
            if(mode==0) {auto dv_kick=dv_gas*All.cf_atime; P.Vel[i]+=dv_kick; P.dp[i]+=dv_kick*P.Mass[i];} else {CellP.VelPred[i] += dv_gas*All.cf_atime;} // update gas velocities (radiation pressure forces here)
            if(mode==0) {CellP.InternalEnergy[i] += de_gas_internal;} else {CellP.InternalEnergyPred[i] += de_gas_internal;} // update gas internal energy (work terms, after subtracting kinetic energy changes)
            for(k_om=0;k_om<N_RT_INTENSITY_BINS;k_om++) {if(mode==0) {CellP.Rad_Intensity[i][kf][k_om] = i0[k_om]/RT_INTENSITY_BINS_DOMEGA;} else {CellP.Rad_Intensity_Pred[i][kf][k_om] = i0[k_om]/RT_INTENSITY_BINS_DOMEGA;}} // update intensities (all of the above)
            CellP.Rad_E_gamma[i][kf]=egy_f; // set this every time this subroutine is called, so it is accessible everywhere else //
        } // loop over iterations
    } // loop over frequencies
    } // finite timestep requirement
#else
    double mom_fac = 1. - RSOL_CORRECTION_FACTOR_FOR_VELOCITY_TERMS(i) * total_erad_emission_minus_absorption / (P.Mass[i] * C_LIGHT_CODE_REDUCED(i)*C_LIGHT_CODE_REDUCED(i)); // back-reaction on gas from emission, which is isotropic in the fluid frame but anisotropic in the lab frame. this effect is only important in actually semi-relativistic problems so we use "real" C here, not a RSOL, and match the corresponding term above in the radiation flux equation (if that is evolved explicitly). careful checking-through gives the single termm here, not both
    ...
    {int k_dir; for(k_dir=0;k_dir<3;k_dir++) {if(mode==0) {P.dp[i][k_dir]+=P.Vel[i][k_dir]*(mom_fac-1.)*P.Mass[i]; P.Vel[i][k_dir]*=mom_fac;} else {CellP.VelPred[i][k_dir] *= mom_fac;}}}
#endif
...
void rt_apply_boundary_conditions(int i)
{
    ...
    if(DMAX(DMAX(P.Pos[i][0],P.Pos[i][1]),P.Pos[i][2]) > 0.9*All.BoxSize || DMIN(DMIN(P.Pos[i][0],P.Pos[i][1]),P.Pos[i][2]) < 0.1*All.BoxSize)
    {
        for(k = 0; k < N_RT_FREQ_BINS; k++)
        {
            CellP.Rad_E_gamma[i][k] = urad[k] * P.Mass[i]/(CellP.Density[i] * All.cf_a3inv);
#ifdef RT_EVOLVE_FLUX
            for(k_dir = 0; k_dir < 3; k_dir++){CellP.Rad_Flux[i][k][k_dir] = 0;}
#endif
#ifdef RT_INFRARED
            if(k==RT_FREQ_BIN_INFRARED) {
                CellP.Radiation_Temperature[i] = background_isrf_cmb_Teff();
                CellP.Dust_Temperature[i] = DMIN(All.InitRadiationTemp,100.);
            }
#endif
        }
    } else {
        for(k = 0; k < N_RT_FREQ_BINS; k++){CellP.Rad_E_gamma[i][k] = DMAX(CellP.Rad_E_gamma[i][k], MIN_REAL_NUMBER);}
    }
}
...
void rt_set_simple_inits(int RestartFlag)
{
    ...
    int i; for(i = 0; i < NumPart; i++)
    {
        if(P.Type[i] == 0)
        {
            int k;
#ifdef RT_INFRARED
            if(flag_to_reset_values_on_startup) {CellP.Radiation_Temperature[i] = CellP.Dust_Temperature[i] = DMIN(All.InitRadiationTemp,100.);} //get_min_allowed_dustIRrad_temperature(); // in K, floor = CMB temperature or 10K
#ifdef RT_ISRF_BACKGROUND
            if(flag_to_reset_values_on_startup) {CellP.Radiation_Temperature[i] = background_isrf_cmb_Teff();} //CellP.Dust_Temperature[i];
#endif
            ...
            {
                CellP.HII[i] = MIN_REAL_NUMBER;
                CellP.HI[i] = 1.0 - CellP.HII[i];
                CellP.Ne[i] = CellP.HII[i];
#ifdef RT_CHEM_PHOTOION_HE
                double fac = (1-HYDROGEN_MASSFRAC) / (4.0 * HYDROGEN_MASSFRAC);
                CellP.HeIII[i] = MIN_REAL_NUMBER * fac;
                CellP.HeII[i] = MIN_REAL_NUMBER * fac;
                CellP.HeI[i] = (1.0 - CellP.HeII[i] - CellP.HeIII[i]) * fac;
                CellP.Ne[i] += CellP.HeII[i] + 2.0 * CellP.HeIII[i];
#endif
            } else {
#ifdef RT_CHEM_PHOTOION_HE
                CellP.HeIII[i] = DMIN(1.0, DMAX(MIN_REAL_NUMBER, 1.0 - CellP.HeII[i] - CellP.HeI[i] * (4.0 * HYDROGEN_MASSFRAC)/(1-HYDROGEN_MASSFRAC))); // not read in since for this subroutine follows from others
#endif
            }
#endif
#ifdef RT_ISRF_BACKGROUND
            ...
            for(k = 0; k < N_RT_FREQ_BINS; k++)
            {
                if(flag_to_reset_values_on_startup) {CellP.Rad_E_gamma[i][k] = MIN_REAL_NUMBER;}
...
#endif
                if(flag_to_reset_values_on_startup_et) {CellP.ET[i][k].set_isotropic(1./3.);}
                CellP.Rad_Je[i][k] = 0;
#ifdef RT_FLUXLIMITER
                CellP.Rad_Flux_Limiter[i][k] = 1;
#endif
                ...
                if(flag_to_reset_values_on_startup && k==RT_FREQ_BIN_INFRARED) { // only initialize the IR energy if starting a new run, otherwise use what's in the snapshot
                    CellP.Rad_E_gamma[i][RT_FREQ_BIN_INFRARED] = (4.*5.67e-5 / C_LIGHT_CGS) * pow(DMIN(All.InitRadiationTemp,100.),4.) / UNIT_PRESSURE_IN_CGS * P.Mass[i] / (CellP.Density[i]*All.cf_a3inv);
                }
#endif
#ifdef RT_ISRF_BACKGROUND
                if(flag_to_reset_values_on_startup) {CellP.Rad_E_gamma[i][k] = urad[k] * P.Mass[i] / (CellP.Density[i]*All.cf_a3inv);}
#endif
#ifdef RT_EVOLVE_ENERGY
                CellP.Rad_E_gamma_Pred[i][k] = CellP.Rad_E_gamma[i][k]; 
                CellP.Dt_Rad_E_gamma[i][k] = 0;
#endif
#ifdef RT_EVOLVE_FLUX
                ...
                if(flag_to_reset_values_on_startup_flux) {
                    for(k_dir=0;k_dir<3;k_dir++) {CellP.Rad_Flux_Pred[i][k][k_dir] = 0;}
                } else {
                    CellP.Rad_Flux_Pred[i][k] *= P.Mass[i]/(CellP.Density[i]*All.cf_a3inv); // need to correct the units here before using
                }
                CellP.Rad_Flux[i][k] = CellP.Rad_Flux_Pred[i][k];
                CellP.Dt_Rad_Flux[i][k] = {};
#endif
#ifdef RT_EVOLVE_INTENSITIES
                int k_dir; for(k_dir=0;k_dir<N_RT_INTENSITY_BINS;k_dir++) {CellP.Rad_Intensity_Pred[i][k][k_dir] = CellP.Rad_Intensity[i][k][k_dir] = MIN_REAL_NUMBER; CellP.Dt_Rad_Intensity[i][k][k_dir] = 0;}
#endif
                
#ifdef GRAIN_RDI_TESTPROBLEM_LIVE_RADIATION_INJECTION
                double q_a = (0.75*All.Grain_Q_at_MaxGrainSize) / ((All.Grain_Internal_Density/UNIT_DENSITY_IN_CGS)*(All.Grain_Size_Max/UNIT_LENGTH_IN_CGS)), e0 = All.Vertical_Grain_Accel / q_a, kappa_0 = All.Grain_Absorbed_Fraction_vs_Total_Extinction * q_a * All.Dust_to_Gas_Mass_Ratio, cell_vol = (P.Mass[i]/CellP.Density[i]);
...
#endif
                double E_cell_thin = cell_vol * e0 * exp(-kappa_0*rho_base_setup*H_scale_setup*(1.-exp(-P.Pos[i][2]/H_scale_setup))), E_cell=E_cell_thin; // attenuate according to equilibrium expectation, if we're using single-scattering radiation pressure [otherwise comment this line out] //
                double tau_tot = q_a * All.Dust_to_Gas_Mass_Ratio * rho_base_setup*H_scale_setup; if(tau_tot>1) {E_cell = cell_vol * (3.*All.Vertical_Grain_Accel*All.Dust_to_Gas_Mass_Ratio*rho_base_setup*H_scale_setup) * (exp(-P.Pos[i][2]/H_scale_setup) + 1./tau_tot);} // attenuate according to approximate optically-thick expression with free-streaming from the 'photosphere' when optically thin
                CellP.Rad_E_gamma_Pred[i][k] = CellP.Rad_E_gamma[i][k] = E_cell;
#if defined(RT_EVOLVE_FLUX)
                CellP.Rad_Flux_Pred[i][k][2]=CellP.Rad_Flux[i][k][2] = E_cell_thin*C_LIGHT_CODE_REDUCED(i);
                CellP.Rad_Flux[i][k][0]=CellP.Rad_Flux[i][k][1]=CellP.Rad_Flux_Pred[i][k][0]=CellP.Rad_Flux_Pred[i][k][1]=0;
#endif
#endif
                CellP.Rad_Kappa[i][k] = rt_kappa(i,k); // let everything else get reset first before calling this //
            }
        }
    }
}
...
void rt_get_lum_gas(int target, double *je)
{
    ...
    double t_eff = 0.59 * (GAMMA(target)-1.) * U_TO_TEMP_UNITS * CellP.InternalEnergyPred[target]; // we're assuming fully-ionized gas with a simple equation-of-state here, nothing fancy, to get the temperature //
    je[k] = rt_absorb_frac_albedo(target,k) * rt_kappa(target,k) * P.Mass[target] * ((4. * 5.67e-5) * t_eff*t_eff*t_eff*t_eff) / UNIT_FLUX_IN_CGS; // blackbody emissivity (Kirchoff's law): account for albedo [absorption opacity], and units //
#endif
}
...
double dust_dE_cooling(int i, double Tgas, double Tdust, double* Tdust_fixedpoint_1, double* Tdust_fixedpoint_2){
    ...
    double nHcgs = HYDROGEN_MASSFRAC * UNIT_DENSITY_IN_CGS * CellP.Density[i] * All.cf_a3inv / PROTONMASS_CGS;
    double lambda_to_dErad = (C_LIGHT_CODE_REDUCED(i)/C_LIGHT_CODE) * nHcgs * nHcgs * (dt*UNIT_TIME_IN_CGS) / (CellP.Density[i] * All.cf_a3inv * UNIT_DENSITY_IN_CGS) / (UNIT_SPECEGY_IN_CGS) * P.Mass[i]; /* need to account for RSOL factors in emission/absorption rates */
    ...
    for(int k=0; k < N_RT_FREQ_BINS; k++){
        ...
        double e_final = CellP.Rad_E_gamma[i][k] + CellP.Lambda_RadiativeCooling_toRHDBins[i][k] * lambda_to_dErad;
    ...
    double LambdaIR_gas = CellP.Lambda_RadiativeCooling_toRHDBins[i][RT_FREQ_BIN_INFRARED];
    ...
    double fac_emission = 4.*5.67e-5/(UNIT_PRESSURE_IN_CGS*UNIT_VEL_IN_CGS)*P.Mass[i]*(C_LIGHT_CODE_REDUCED(i)/C_LIGHT_CODE)*dt;
    double dust_emission = fac_emission*kappa_dust_emission*pow(Tdust,4); // *total* dust emission
    
    double T_IR_0 = CellP.Radiation_Temperature[i];
    double Tmax = DMAX(DMAX(Tgas, Tdust),T_IR_0), Tmin = DMIN(DMIN(Tgas,Tdust), T_IR_0);
    double e_IR_0 = CellP.Rad_E_gamma[i][RT_FREQ_BIN_INFRARED];
    ...
    //}}
#ifdef COOLING
    CellP.Radiation_Temperature_CoolingWeighted[i] = T_IR_final; // PFH: this had been protected by a Tdust < max, but that will give the wrong radiation temperature when radiation-gas (non dust) interactions like Compton or Kramers are important, needs to be set regardless of whether the dust itself is present
#endif
    ...
    dust_absorption += e_IR_final * C_LIGHT_CODE_REDUCED(i) * rt_kappa_adaptive_IR_band(i, Tdust, T_IR_final,-1,1) * CellP.Density[i]*All.cf_a3inv * dt;
...
double rt_ir_lambdadust(int i, double T){
    ...
    if((All.Time==0 )|| (!isfinite(CellP.Dust_Temperature[i]))) {Tdust=T;} else {Tdust = DMIN(MAX_DUST_TEMP, CellP.Dust_Temperature[i]);}

    dE = dE_guess = ROOTFIND_FUNCTION_INNER(Tdust-T);
    //if(CellP.Dust_Temperature[i] >= MAX_DUST_TEMP && dE > 0) {return 0;}
    ...
    if(T_upper>=MAX_DUST_TEMP && dE_upper > 0) {CellP.Dust_Temperature[i] = MAX_DUST_TEMP; return 0;}

    if(dE_lower * dE_upper > 0) {PRINT_WARNING("Failed to bracket Tdust solution for ID=%lld T=%g T_lower=%g T_upper=%g dE_lower=%g dE_upper=%g\n", (long long)P.ID[i], T, T_lower,T_upper, dE_lower, dE_upper);}
    ...
    CellP.Lambda_RadiativeCooling_toRHDBins[i][RT_FREQ_BIN_INFRARED] += LambdaDust;
    CellP.Dust_Temperature[i] = Tdust;
    return LambdaDust;
}
...
{
    double nHcgs = HYDROGEN_MASSFRAC * UNIT_DENSITY_IN_CGS * CellP.Density[i] * All.cf_a3inv / PROTONMASS_CGS;    /* hydrogen number dens in cgs units */
    double fac_emission = 4.*5.67e-5/(UNIT_PRESSURE_IN_CGS*UNIT_VEL_IN_CGS)*CellP.Density[i]*All.cf_a3inv; // in code units
    ...
    double column = evaluate_NH_from_GradRho(CellP.Gradients.Density[i],P.KernelRadius[i],CellP.Density[i],P.NumNgb[i],1,i);
...
#ifdef RT_INFRARED
    Tdust_guess = DMIN(DMAX(CellP.Dust_Temperature[i], 1.), MAX_DUST_TEMP); // previous dust temperature should be a good guess
#else // case where we don't have a pre-computed dust temp, use asymptotic limits to get a good guess
    double Zfac = 1.0;
#ifdef METALS
    if(i>=0) {Zfac = P.Metallicity[i][0]/All.SolarAbundances[0];}
#endif
    double rho_c_arad_fac = (4.*5.67e-5)/(UNIT_VEL_IN_CGS*UNIT_PRESSURE_IN_CGS)*CellP.Density[i]*All.cf_a3inv; // a c rho in code units
...
{
#if defined(RT_INFRARED)
    double u_gamma = CellP.Rad_E_gamma[i][RT_FREQ_BIN_INFRARED] * (CellP.Density[i]*All.cf_a3inv/P.Mass[i]) * blackbody_lum_frac(E_lower, E_upper, CellP.Radiation_Temperature[i]);
...
double stellar_lum_in_band(int i, double E_lower, double E_upper)
{
    ...
    double r_sol = P.ProtoStellarRadius_inSolar[i], l_sol = P.StarLuminosity_Solar[i];
#elif defined(SINGLE_STAR_SINK_DYNAMICS) // use generic fits based on mass
    double l_sol=sink_lum_bol(0,P.Mass[i],i)*UNIT_LUM_IN_SOLAR, m_sol=P.Mass[i]*UNIT_MASS_IN_SOLAR, r_sol=pow(m_sol,0.738); // L/Lsun, M/Msun, R/Rsun
#else
...
int rt_get_source_luminosity_chimes(int i, int mode, double *lum, double *chimes_lum_G0, double *chimes_lum_ion)
{
    ...
    if( ((P.Type[i] == 4)||((All.ComovingIntegrationOn==0)&&((P.Type[i] == 2)||(P.Type[i]==3)))) && (P.Mass[i]>0) && (P.KernelRadius[i]>0) )
    {
        int age_bin, j; double age_Myr=1000.*evaluate_stellar_age_Gyr(i), log_age_Myr=log10(age_Myr), stellar_mass=P.Mass[i]*UNIT_MASS_IN_SOLAR;
...
double rt_kappa_adaptive_IR_band(int i, double T_dust, double Trad, int do_emission_absorption_scattering_opacity, int dust_or_gas_opacity_only_flag)
{
    ...
    if(dust_or_gas_opacity_only_flag <= 0) // non-dust (e.g. gas-phase) IR opacities
    {
...
#ifdef COOLING
        x_elec = CellP.Ne[i]; // actual free electron fraction
#endif
#ifdef METALS
        zmetals = P.Metallicity[i][0];
#endif
        ...
        double Tgas=1. + 0.59*(GAMMA(i)-1.)*U_TO_TEMP_UNITS*CellP.InternalEnergyPred[i], rho_cgs = CellP.Density[i]*All.cf_a3inv*UNIT_DENSITY_IN_CGS; /* crude estimate of gas temperature to use with scalings below, and gas density in cgs */
        ...
        k_molecular *= CellP.MolecularMassFraction[i];
#endif
...
#ifdef COOLING
#ifdef RT_CHEM_PHOTOION
        double x_Hp = CellP.HII[i], x_H0 = CellP.HI[i];
#else
        double u_in=CellP.InternalEnergy[i], rho_in=CellP.Density[i]*All.cf_a3inv, mu=1, ne=1, nHI=0, nHII=0, nHeI=1, nHeII=0, nHeIII=0;
