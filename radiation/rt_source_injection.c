    ...
    while(startnode >= 0)
    {
		...
		if(P[j].StellarAge==All.Time) {continue;} // This is a wind cell that was just spawned, and is not yet part of the volume partition, so don't inject // 
                ...
                for(k=0;k<N_RT_FREQ_BINS;k++) 
                {
                    ...
                    if((k >= RT_FREQ_BIN_H0) && (k < RT_FREQ_BIN_H0 + RT_CHEM_PHOTOION)) { // do this block for all ionizing bins
                        double stellum=0;
			if(local.Dt > 0) {
			    int k2; for(k2=RT_FREQ_BIN_H0; k2 < RT_FREQ_BIN_H0 + RT_CHEM_PHOTOION; k2++) { stellum += local.Luminosity[k2]; } // add up total ionizing photon energy
