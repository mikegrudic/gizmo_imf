...
double rt_DoHeating(int i, double dt)
{
    ...
    nH = HYDROGEN_MASSFRAC * CellP[i].Density * All.cf_a3inv / PROTONMASS_CGS * UNIT_MASS_IN_CGS;
    nHI = CellP[i].HI * nH;
    sigma = 1.63e-18 / (UNIT_LENGTH_IN_CGS*UNIT_LENGTH_IN_CGS);
    e_gamma = CellP[i].Rad_E_gamma[0] * (CellP[i].Density*All.cf_a3inv/P[i].Mass);  // want the photon energy density //
    rate = nHI * C_LIGHT_CODE * sigma * e_gamma;
    return rate / (CellP[i].Density * All.cf_a3inv);
}
...
#ifdef RT_CHEM_PHOTOION_HE
    nHeI = CellP[i].HeI * nH;
    nHeII = CellP[i].HeII * nH;
    nHeIII = CellP[i].HeIII * nH;
#endif
...
double rt_DoCooling(int i, double dt)
{
    ...
    double internal_energy = CellP[i].InternalEnergyPred;
    
    /* do the cooling */
    double lambda = rt_get_cooling_rate(i, internal_energy);
    double de = lambda * dt / (CellP[i].Density * All.cf_a3inv);
    ...
    else
    {
        ...
        ratefact = dt / (CellP[i].Density * All.cf_a3inv);
        iter = 0;
...
double rt_get_cooling_rate(int i, double internal_energy)
{
    ...
    nH = HYDROGEN_MASSFRAC * CellP[i].Density * All.cf_a3inv * UNIT_MASS_IN_CGS / PROTONMASS_CGS;	//physical
