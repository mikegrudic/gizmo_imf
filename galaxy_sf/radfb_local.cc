...
void radiation_pressure_winds_consolidated(void)
{
    ...
    {
        if((P.Type[i] == 4)||((All.ComovingIntegrationOn==0)&&((P.Type[i] == 2)||(P.Type[i]==3))))
        {
            if(P.Mass[i] <= 0 || !isfinite(P.Mass[i])) {continue;};
            double star_age = evaluate_stellar_age_Gyr(i);
            if( (star_age < age_threshold_in_gyr) && (P.Mass[i] > 0) && (P.DensityAroundParticle[i] > 0) )
            {
                ...
                double lum_cgs = (lm_ssp * SOLAR_LUM_CGS) * (P.Mass[i]*UNIT_MASS_IN_SOLAR); // total L in CGS of star particle
                ...
                double RtauMax = P.KernelRadius[i] * (5. + 2.0 * rt_kappa(i,RT_FREQ_BIN_FIRE_UV) * P.KernelRadius[i]*P.DensityAroundParticle[i]*All.cf_a2inv); // guess search radius which is a few H, plus larger factor if optically thick //
...
#endif
                double rho_phys=P.DensityAroundParticle[i]*All.cf_a3inv, h_phys=P.KernelRadius[i]*All.cf_atime; // density and h in -physical- units
                double v_grav_guess; v_grav_guess = DMIN( 1.82*(65.748/UNIT_VEL_IN_KMS)*pow(1.+rho_phys*UNIT_DENSITY_IN_NHCGS,-0.25) , sqrt(All.G*(P.Mass[i] + VOLUME_NORM_COEFF_FOR_NDIMS*rho_phys*h_phys*h_phys*h_phys)/h_phys) ); // don't want to 'under-kick' if there are small local characteristic velocities in the region of interest
                ...
                double dv_imparted_perpart_guess = (dE_over_c/P.Mass[i]); // estimate of summed dv_imparted [in code units] from single-scattering: = momentum/mass of particle
                ...
                dv_imparted_perpart_guess += (dE_over_c/P.Mass[i]) * tau_IR_guess; // estimate of additional IR term [1+tau_IR]*L/c assumed here as coupling //
                ...
                if((P.Type[i]==4) && (P.NewStar_Momentum_For_JetFeedback[i]>0)) {prob = 1; jet_momentum_tocouple = P.NewStar_Momentum_For_JetFeedback[i];} // require entering loop with desired momentum budget in addition to usual
...
#endif
                double p_random = get_random_number(P.ID[i]+ThisTask+i+2); // random number for use below
                if(p_random <= prob) // alright, its worth doing the loop!
                { // within loop
                    /* ok, now open the neighbor list for the star particle */
                    N_MIN_KERNEL=10; N_MAX_KERNEL=256; MAXITER_FB=100; h=1.0*P.KernelRadius[i];
                    NITER=0; wt_sum=0; startnode=All.MaxPart; dummy=0; numngb_inbox=0; pos=P.Pos[i];
...
#endif
                                if((P.Mass[j]>0) && (CellP.Density[j]>0))
                                {
                    ...
                    if(wt_sum > 0)  /* found at least one massive neighbor, can proceed */
                    {
                        for(n=0; n<numngb_inbox; n++)
                        {
                            ...
                            if((P.Mass[j]>0) && (CellP.Density[j]>0))
                            {
                                ...
                                double dv_imparted_singlescattering = wk * (dE_over_c / P.Mass[j]); // fractional initial photon momentum seen by this neighbor
#if (GALSF_FB_FIRE_STELLAREVOLUTION > 2)
                                /* estimate fraction of the available single-scattering RP that can actually be absorbed in the cell */
                                double sigma_cell_to_total = (1./wk) * (P.Mass[j] / (h_eff_j*All.cf_atime * h_eff_j*All.cf_atime)); // code units -- correct back to 'total' column through all neighbors, since thats what determines the total fraction that will be absorbed here //
                                ...
                                if(jet_momentum_tocouple > 0) {
                                    dv_imparted_singlescattering += wk * jet_momentum_tocouple / P.Mass[j]; // add extra momentum per our setup
                                    P.NewStar_Momentum_For_JetFeedback[i] -= wk * jet_momentum_tocouple;} // remove it from the budget
#endif
                                ...
                                double dv_imparted_multiplescattering = All.RP_Local_Momentum_Renormalization * (dE_over_c / P.Mass[j]) * kappa_ir_codeunits * (P.Mass[j]/(4.*M_PI*r2*All.cf_atime*All.cf_atime));
#if (GALSF_FB_FIRE_STELLAREVOLUTION > 2)
                                ...
                                if(n>0) {p_random=get_random_number(P.ID[j]+P.ID[i] +ThisTask+ 3);} //else p_random=0;
                                if(p_random < prob)
#endif
                                { /* open subloop with wind kick */
                                    ...
                                    total_n_wind += 1.0; total_mom_wind += P.Mass[j]*delta_v_imparted_rp; avg_v_kick += delta_v_imparted_rp;
                                    avg_taufac +=  (P.Mass[j]*delta_v_imparted_rp) * (dv_imparted_multiplescattering / (dE_over_c / P.Mass[j]));
                                    ...
                                    dir = -P.GradRho[j]; norm = dir.norm_sq(); // based on density gradient near star //
#else
                                    if(dv_imparted_singlescattering > dv_imparted_multiplescattering) {dir = dp; norm = dir.norm_sq();} // if kick is primarily from uv, then orient directly //
                                        else {dir = -P.GradRho[j]; norm = dir.norm_sq();} // otherwise, along opacity gradient //
#endif
                                    if(norm>0) {norm=sqrt(norm); dir /= norm;} else {dir = {0,0,1}; norm=1;}
                                    {auto dv_kick = delta_v_imparted_rp * All.cf_atime * dir; P.Vel[j] += dv_kick; CellP.VelPred[j] += dv_kick; P.dp[j] += dv_kick * P.Mass[j];} /* apply the kick [put into comoving code units as oppropriate */
                                    ...
                                    if(norm>0) {norm=sqrt(norm); dir /= norm;} else {dir = {0,0,1}; norm=1;}
                                    {auto dv_kick = delta_v_imparted_rp * All.cf_atime * dir; P.Vel[j] += dv_kick; CellP.VelPred[j] += dv_kick; P.dp[j] += dv_kick * P.Mass[j];} /* apply the kick */
#endif
                                } /* closes if(get_random_number(P.ID[i] + 2) < prob) */
                            } /* if( (P.Mass[j]>0) && (CellP.Density[j]>0) ) */
                        } /* for(n=0; n<numngb_inbox; n++) */
                    } /* if (rho>0) */
                } // // within loop
            } // star age, mass check:: (star_age < 0.1) && (P.Mass[i] > 0) && (P.DensityAroundParticle[i] > 0)
        } // particle type check::  if((P.Type[i] == 4)....
...
void HII_heating_singledomain(void)    /* this version of the HII routine only communicates with particles on the same processor */
{
    ...
    for (int i : ActiveParticleList)
    {
        ...
        if((P.Type[i] == 5)||(((P.Type[i] == 4)||((All.ComovingIntegrationOn==0)&&((P.Type[i] == 2)||(P.Type[i]==3))))))
#else
        if((P.Type[i] == 4)||((All.ComovingIntegrationOn==0)&&((P.Type[i] == 2)||(P.Type[i]==3))))
#endif
        {
            ...
            stellum = chimes_ion_luminosity(evaluate_stellar_age_Gyr(i)*1000.,P.Mass[i]*UNIT_MASS_IN_SOLAR) * 4.68e-11; // chimes ionizing photon flux rescaled to mean spectrum here appropriately (~29eV per photon)
#endif
            if(stellum <= 0) {continue;}
            pos = P.Pos[i]; rho = P.DensityAroundParticle[i]; h_i = P.KernelRadius[i];
            ...
            prandom = get_random_number(P.ID[i] + 7); // pre-calc the (eventually) needed random number
            // guesstimate if this is even close to being interesting for the particle masses of interest
            if(prandom < 5.0*mionizable/P.Mass[i]) // prandom > this, won't be able to ionize anything interesting
            {
                ...
                do {
                    ...
                    if(numngb>0)
                    {
                        ...
                        for(n = 0; n < numngb; n++)
                        {
                            ...
                            j = ngb_list_touse[n];
                            if(P.Mass[j] <= 0 || P.Type[j] !=0) {continue;}
                            if(CellP.DelayTimeHII[j] > 0) {continue;}
#if (GALSF_FB_FIRE_STELLAREVOLUTION > 2) && !defined(CHIMES_HII_REGIONS)
                            if(CellP.Ne[j] > 0.8) {continue;}
#endif
                            if(P.Type[j] == 0 && P.Mass[j] > 0)
                            {
                                double dx=pos[0]-P.Pos[j][0], dy=pos[1]-P.Pos[j][1], dz=pos[2]-P.Pos[j][2];
                                ...
                                if(CellP.InternalEnergy[j]<CellP.InternalEnergyPred[j]) {u=CellP.InternalEnergy[j];} else {u=CellP.InternalEnergyPred[j];}
                                if(CellP.DelayTimeHII[j] > 0) {already_ionized=1;}
#if !defined(CHIMES_HII_REGIONS)
#if (GALSF_FB_FIRE_STELLAREVOLUTION > 2) 
                                if((CellP.Ne[j]>0.8) || (u>50.*uion)) {already_ionized=1;} /* already mostly ionized by formal ionization fraction */
#else
                                ...
                                {
                                    m_effective = P.Mass[j]*(CellP.Density[j]/rho);
                                    ...
                                    if(do_ionize==1)
                                    {
                                        ...
                                        total_N_ionized += 1;
                                        mion_actual += P.Mass[j];
                                        avg_RHII += P.Mass[j]*r*All.cf_atime*UNIT_LENGTH_IN_KPC;
                                    }
...
#if (GALSF_FB_FIRE_STELLAREVOLUTION > 2) 
                                if((CellP.Density[j]<rnearest)&&(already_ionized==0)) {rnearest = CellP.Density[j]; jnearest = j;} // rank by density, not distance
#else
                    ...
                    {
                        j=jnearest; m_effective=P.Mass[j]*(CellP.Density[j]/rho); m_available=mionizable-mionized; prob=m_available/m_effective; do_ionize=0;
                        if(prandom < prob) {do_ionize=1;}
                        if(do_ionize==1)
                        {
                            already_ionized = do_the_local_ionization(j,dt,i);
                            total_N_ionized += 1;
                            mion_actual += P.Mass[j];
                            double dx=pos[0]-P.Pos[j][0],dy=pos[1]-P.Pos[j][1],dz=pos[2]-P.Pos[j][2],r2; NEAREST_XYZ(dx,dy,dz,1); r2=dx*dx+dy*dy+dz*dz;
                            avg_RHII += P.Mass[j]*sqrt(r2)*All.cf_atime*UNIT_LENGTH_IN_KPC;
                        }
                        mionized += prob*m_effective;
                    } // if((mionized<mionizable)&&(jnearest>=0))
            ...
            } // if(prandom < 2.0*mionizable/P.Mass[j])
        } // if((P.Type[i] == 4)||(P.Type[i] == 2)||(P.Type[i] == 3))
    } // for (int i : ActiveParticleList)
...
int do_the_local_ionization(int target, double dt, int source)
{
    ...
    for(k=0;k<CHIMES_LOCAL_UV_NBINS;k++) {CellP.Chimes_fluxPhotIon_HII[target][k]=0; CellP.Chimes_G0_HII[target][k]=0;}
    // set the quantities desired for this age bin specifically: need a softened radius, for use here //
    double stellar_mass=P.Mass[source]*UNIT_MASS_IN_SOLAR; Vec3<double> dp = P.Pos[source] - P.Pos[target];
    ...
    CellP.Chimes_fluxPhotIon_HII[target][age_bin] = (1.0 - All.Chimes_f_esc_ion) * chimes_ion_luminosity(stellar_age_myr, stellar_mass) / r2; // cgs flux of H-ionising photons per second seen by the star particle
    CellP.Chimes_G0_HII[target][age_bin] = (1.0 - All.Chimes_f_esc_G0) * chimes_G0_luminosity(stellar_age_myr, stellar_mass) / r2; // cgs flux in the 6-13.6 eV band

#else

#if (GALSF_FB_FIRE_STELLAREVOLUTION <= 2) 
    CellP.InternalEnergy[target] = DMAX(CellP.InternalEnergy[target] , HIIRegion_Temp / (0.59 * (5./3.-1.) * U_TO_TEMP_UNITS)); /* assume fully-ionized gas with gamma=5/3 */
    CellP.InternalEnergyPred[target] = CellP.InternalEnergy[target]; /* full reset of the internal energy */
#else
    double delta_U_of_ionization = (20.-13.6) * ((ELECTRONVOLT_IN_ERGS / PROTONMASS_CGS) / UNIT_SPECEGY_IN_CGS) * (1.-DMAX(0.,DMIN(1.,CellP.Ne[target]/1.5))); /* energy injected per unit mass, in code units, by ionization, assuming each atom absorbs, and mean energy of absorbed photons is given by x=18 eV here (-13.6 for energy of ionization) */
...
#ifdef METALS
    Z_sol = P.Metallicity[target][0]/All.SolarAbundances[0]; // set metallicity
#if defined(GALSF_ISMDUSTCHEM_MODEL)
    Z_sol = (P.Metallicity[target][4]-CellP.ISMDustChem_Dust_Metal[target][4])/All.SolarAbundances[4]; // cooling for HII equilibrium depends predominantly on gas-phase O so need to account for any locked up in dust
#endif
#endif
    ...
    double u_post_ion_no_cooling = CellP.InternalEnergy[target] + delta_U_of_ionization; // energy after ionization, before any cooling
    double u_final = DMIN( u_post_ion_no_cooling , u_eqm ); // don't heat to higher temperature than intial energy of ionization allows
    CellP.InternalEnergy[target] = u_final; CellP.InternalEnergyPred[target] = u_final; /* add it */
    ...
    double n1000 = CellP.Density[target]*All.cf_a3inv*UNIT_DENSITY_IN_NHCGS / 1000.; // density in 1000 cm^-3
    ...
    CellP.Rad_Flux_UV[target] += flux_compactHII; CellP.Rad_Flux_EUV[target] += flux_compactHII;
#endif
    CellP.Ne[target] = 1.0 + 2.0*yhelium(target); /* set the cell to fully ionized */

#endif

    CellP.DelayTimeHII[target] = DMIN(dt, 10./UNIT_TIME_IN_MYR); /* tell the code to flag this in the cooling subroutine */
    return 1;
}
