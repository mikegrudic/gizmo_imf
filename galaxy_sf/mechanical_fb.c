...
int addFB_evaluate(int target, int mode, int *exportflag, int *exportnodecount, int *exportindex, int *ngblist, int loop_iteration)
{
    ...
    double sedov_phase_thermal_to_kinetic_ratio = 2.54, f_sedov_kin, thermal_kinetic_ratio_lim_wvinwards = 0; // define ratio of thermal-to-kinetic energy for sedov-taylor phase
...
#ifdef GALSF_FB_FIRE_STELLAREVOLUTION
    thermal_kinetic_ratio_lim_wvinwards = sedov_phase_thermal_to_kinetic_ratio; sedov_phase_thermal_to_kinetic_ratio = 1.e-2; // for range of resolution we want to model, only treat the kinetic component initially (include thermal component in residual thermal energy term)
    thermal_kinetic_ratio_lim_wvinwards = sedov_phase_thermal_to_kinetic_ratio; // current 'default' fire-3 behavior for runs prior to Feb 20, 2022: testing here
#endif
    ...
    if((local.Area_weighted_sum[0] > MIN_REAL_NUMBER) && (loop_iteration >= 0))
    {
        ...
        if(beta_cool > 0.5*psi_thold) {psi_cool = psi_thold/(2.*beta_cool);} else {psi_cool = psi_thold;} // for cooling limit, only need upper limit to psi, all else will use less energy
    }
