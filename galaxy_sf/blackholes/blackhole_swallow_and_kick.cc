...
static inline void INPUTFUNCTION_NAME(struct INPUT_STRUCT_NAME *in, int i, int loop_iteration)
{
    ...
    in->Hsml = P[i].Hsml; in->Mass = P[i].Mass; in->BH_Mass = P[i].BH_Mass; in->ID = P[i].ID; in->ID_child_number=P[i].ID_child_number; in->ID_generation=P[i].ID_generation; in->Mdot = P[i].BH_Mdot;
#if defined(BH_CALC_LOCAL_ANGLEWEIGHTS) || defined(BH_WIND_KICK)
...
#endif
#ifdef BH_ALPHADISK_ACCRETION
    in->BH_Mass_AlphaDisk = P[i].BH_Mass_AlphaDisk;
#endif
#if defined(BH_CALC_LOCAL_ANGLEWEIGHTS)
...
#endif
#ifdef BH_COUNTPROGS
    P[i].BH_CountProgs += out->BH_CountProgs;
#endif
#ifdef GALSF
        ...
        while(startnode >= 0) {
            ...
            for(n = 0; n < numngb; n++)
            {
                ...
                {   /* accreted quantities to be added [regardless of particle type] */
                    ...
                    if(P[j].Type != 5) {mcount_for_conserve=0;} else {mcount_for_conserve=P[j].BH_Mass;}
#ifdef BH_ALPHADISK_ACCRETION
                    if(P[j].Type == 5) {mcount_for_conserve += P[j].BH_Mass_AlphaDisk;}
#endif
#endif
#ifdef GRAIN_FLUID
                    ...
                    if(P[j].Type == 5) {for(k=0;k<3;k++) {out.accreted_J[k] += (mcount_for_conserve * P[j].BH_Specific_AngMom[k]);}}
#endif
                    ...
                    if(P[j].Type == 5)  /* this is a BH-BH merger */
                    {
                        ...
                        fprintf(FdBhMergerDetails,"%.16g  %llu %g %2.16g %2.16g %2.16g  %llu %g %2.16g %2.16g %2.16g\n", All.Time,  (unsigned long long)local.ID,local.BH_Mass,local.Pos[0],local.Pos[1],local.Pos[2],  (unsigned long long)P[j].ID,P[j].BH_Mass,P[j].Pos[0],P[j].Pos[1],P[j].Pos[2]); fflush(FdBhMergerDetails);
#elif defined(OUTPUT_ADDITIONAL_RUNINFO)
                        fprintf(FdBlackHolesDetails,"ThisTask=%d, time=%.16g: id=%llu swallows %llu (%g %g)\n", ThisTask, All.Time, (unsigned long long)local.ID, (unsigned long long)P[j].ID, local.BH_Mass, P[j].BH_Mass); fflush(FdBlackHolesDetails);
#endif
                        ...
                        /* the true dynamical mass of the merging BH is Mass_j/BH_INCREASE_DYNAMIC_MASS unless exceeded by physical growth
                         - in the limit P[j].BH_Mass > BH_INCREASE_DYNAMIC_MASS x m_b, then bh_mass=Mass_j on average and we are good as well  */
                        out.accreted_Mass    += ( DMAX(P[j].BH_Mass, Mass_j/BH_INCREASE_DYNAMIC_MASS) );
#else
                        out.accreted_Mass    += (Mass_j);
#endif
                        out.accreted_BH_Mass += (P[j].BH_Mass);
#if defined(BH_SWALLOWGAS) && !defined(BH_GRAVCAPTURE_GAS)
                        out.BH_AccretionDeficit += (P[j].BH_AccretionDeficit);
#endif
#ifdef BH_ALPHADISK_ACCRETION
                        out.accreted_BH_Mass_alphadisk += (P[j].BH_Mass_AlphaDisk);
#endif
#ifdef BH_WIND_SPAWN
                        out_accreted_BH_Mass_alphaornot += (P[j].unspawned_wind_mass);
#endif
#ifdef BH_COUNTPROGS
                        out.BH_CountProgs += P[j].BH_CountProgs;
#endif
                        bin = P[j].TimeBin;
                        #pragma omp atomic
                        ...
                        P[j].BH_Mass_AlphaDisk = 0; // make sure the mass is -actually- zero'd here
#endif
                        #pragma omp atomic write
                        P[j].BH_Mdot = 0; // make sure the mass is -actually- zero'd here
                        #pragma omp atomic write
                        P[j].BH_Mass = 0; // make sure the mass is -actually- zero'd here
#ifdef GALSF
                    ...
                    if((P[j].Type > 0) && (P[j].Type < 5))
                    {
                        ...
                        if((P[j].Type == 1) || (All.ComovingIntegrationOn && (P[j].Type==2||P[j].Type==3)) ) { /* this is a DM particle: In this case, no kick, so just zero out the mass and 'get rid of' the particle (preferably by putting it somewhere irrelevant) */
                            ...
                            out.accreted_BH_Mass += (Mass_j); /* if using simple excision, adds to the particle mass, but not assumed to actually be accreted */
#endif
                            #pragma omp atomic
                            N_dm_swallowed++;
...
#ifndef BH_EXCISION_NONGAS
                            out_accreted_BH_Mass_alphaornot += (Mass_j); /* if using simple excision, adds to the particle mass, but not assumed to actually be accreted */
#endif
                            #pragma omp atomic
                            N_star_swallowed++;
                        }
                    ...
                    if(P[j].Type == 0)
                    {
                        ...
                        out_accreted_BH_Mass_alphaornot += (f_accreted*Mass_j);
#endif
#if defined(BH_SWALLOWGAS) && !defined(BH_GRAVCAPTURE_GAS)
                        out.BH_AccretionDeficit -= (f_accreted*Mass_j); /* account for this in the 'continuous accretion' budget, since it is part of the continuous Mdot onto the BH */
#endif
...
void spawn_bh_wind_feedback(void)
{
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        ...
        if((NumPart+n_particles_split+(int)(2.*(BH_WIND_SPAWN+0.1)) < nmax) && (ptype_can_spawn==1)) // basic condition: particle is a 'spawner' (sink), and code can handle the event safely without crashing.
        {
            ...
            if(P[i].unspawned_wind_mass >= (BH_WIND_SPAWN)*target_mass_for_wind_spawning(i)) {sink_eligible_to_spawn=1;} // have 'enough' mass to spawn
...
#endif
                if(P[i].BH_Mass > 0 && P[i].BH_Mass > P[i].Sink_Formation_Mass) {P[i].ProtoStellarStage == 7;} // this is a relic now, move it to the next stage
	    }
	}
    }
#if (defined(SINGLE_STAR_FB_SNE) && defined(SINGLE_STAR_STARFORGE_PROTOSTELLAR_EVOLUTION)) || defined(SINGLE_STAR_FB_SNE_N_EJECTA_QUADRANT)
...
#ifdef BH_DEBUG_SPAWN_JET_TEST
        P[j].Hsml=5.*d_r; CellP[j].Density=mass_of_new_particle/pow(KERNEL_CORE_SIZE*P[j].Hsml,NUMDIMS); /* PFH: need to write this in a way that does not make assumptions about units/problem structure */
#endif
