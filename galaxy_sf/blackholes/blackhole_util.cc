...
double evaluate_blackhole_radiative_efficiency(double mdot, double mass, long pindex)
{
    ...
    double lambda_0 = 0.01, lambda_1 = 2., lambda_eff = mdot/bh_eddington_mdot(mass), qfac = lambda_eff/lambda_0, qfac_he = lambda_eff/lambda_1;
...
double evaluate_blackhole_cosmicray_efficiency(double mdot, double mass, long pindex)
{
    ...
    if(mdot/bh_eddington_mdot(mass) < 0.01) {return 0.5;} else {return 0.1;} /* high-Mdot uses standard SNe kinetic power, otherwise give unity power here */
    ...
    // return All.BH_CosmicRay_Injection_Efficiency * (1. + enhancement_fac/qfac); // older deprecated experiment
#endif
...
#endif
        P[n].BH_Mdot=0;  /* always initialize/default to zero accretion rate */
...
int is_star_eligible_for_binary_merge_away(int j)
{
    ...
    if(P[j].BH_Mass < 2.*P[j].BH_Mdot*star_age) {merge_key = 0;} /* dont allow mergers if the star is still growing very rapidly */
    return merge_key;
}
#endif
