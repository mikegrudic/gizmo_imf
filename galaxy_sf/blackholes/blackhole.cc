...
int bh_check_boundedness(int j, double vrel, double vesc, double dr_code, double sink_radius) // need to know the sink radius, which can be distinct from both the softening and search radii
{
    ...
    if(v2 < 1)
    {
        ...
        if(P[j].Type == 0) {r_j = DMAX(r_j , P[j].Hsml);}
...
double bh_angleweight_localcoupling(int j, double cos_theta, double r, double H_bh)
{
    ...
    double H_j = P[j].Hsml;
    if((r >= H_j && r >= H_bh) || (H_j <= 0) || (P[j].Mass <= 0) || (CellP[j].Density <= 0)) return 0;
...
void set_blackhole_mdot(int i, int n, double dt)
{
    ...
    double meddington = bh_eddington_mdot(P[n].BH_Mass);
#endif
    ...
    if(BlackholeTempInfo[i].Mgas_in_Kernel > 0)
    {
        ...
        bh_mass += P[n].BH_Mass_AlphaDisk;
#endif
        rmax_for_bhar = P[n].Hsml * All.cf_atime; /* convert to physical units */
        ...
        if((bh_mass <=0)||(fgas_for_bhar<=0)||(m_tmp_for_bhar<=0))
        {
            ...
            mdot = 4.*M_PI * All.G*All.G * P[n].BH_Mass*menc_all * (P[n].DensAroundStar*All.cf_a3inv) / pow(soundspeed2 + Vc2_rmax, 1.5);
            ...
            double veldisp2_eff = bhvel2/3. + soundspeed2, masscorrfac = pow( menc_all/(1.e-10*menc_all + P[n].BH_Mass) , 0.25 );
            mdot = masscorrfac * 4.*M_PI * All.G*All.G * P[n].BH_Mass*menc_all * (P[n].DensAroundStar*All.cf_a3inv) / pow(1.e-5*soundspeed2 + Vc2_rmax, 1.5);
            ...
            if(j_tmp_for_bhar < jcirc_crit) /* circularization within BH-dominated region, Bondi accretion valid */
            {
                ...
                mdot = 4.*M_PI * All.G*All.G * P[n].BH_Mass*P[n].BH_Mass * rho / vcs_fac;
...
#endif
        mdot = 4. * M_PI * AccretionFactor * All.G * All.G * P[n].BH_Mass * P[n].BH_Mass * rho / fac;
    }
...
/* DAA: note that we should have mdot=0 here , otherwise the mass accreted is counted twice [mdot*dt in set_blackhole_new_mass, accreted_BH_mass in blackhole_swallow_and_kick] */
    ...
    double x_MdiskSelfGravLimiter = P[n].BH_Mass_AlphaDisk / (BH_ALPHADISK_ACCRETION * P[n].BH_Mass);
    ...
    if(P[n].BH_Mass_AlphaDisk > 0)
    {
        /* this below is a more complicated expression using the outer-disk expression from Shakura & Sunyaev. Simpler expression
           ...
           mdot = (2.45/(UNIT_MASS_IN_SOLAR/UNIT_TIME_IN_YR)) * pow( 0.1 , 8./7.) * // normalization, then viscous disk 'alpha'
             pow( P[n].BH_Mass*UNIT_MASS_IN_SOLAR/1.e8 , -5./14. ) * pow( P[n].BH_Mass_AlphaDisk*UNIT_MASS_IN_SOLAR/1.e8 , 10./7. ) * // mbh , m_disk dependence
             pow( DMIN(0.2,DMIN(P[n].Hsml,SinkParticle_GravityKernelRadius)*All.cf_atime*UNIT_LENGTH_IN_PC) , -25./14. ); // r_disk dependence */
        double t_acc_disk = (4.2e7/UNIT_TIME_IN_YR) * pow((P[n].BH_Mass_AlphaDisk+P[n].BH_Mass) / P[n].BH_Mass_AlphaDisk, 0.4); /* shakura-sunyaev disk, integrated out to Q~1 radius, approximately */
#ifdef BH_TIMESCALE_SET
        double f_disk_bh = P[n].BH_Mass_AlphaDisk / P[n].BH_Mass; t_acc_disk = (BH_TIMESCALE_SET/UNIT_TIME_IN_YR) / sqrt( f_disk_bh * (1 + f_disk_bh) );
#endif
        ...
        mdot = P[n].BH_Mass_AlphaDisk / t_acc_disk;
    }
    ...
    if(dt>0)
    {
        ...
        if(mdot > P[n].BH_Mass_AlphaDisk/dt*All.BAL_f_accretion) mdot = P[n].BH_Mass_AlphaDisk/dt*All.BAL_f_accretion;
#else
        if(mdot > P[n].BH_Mass_AlphaDisk/dt) mdot = P[n].BH_Mass_AlphaDisk/dt;
#endif
    }
    ...
    for(k=0;k<3;k++) {jmag+=BlackholeTempInfo[i].angmom_prepass_sum_for_passback[k]*BlackholeTempInfo[i].angmom_prepass_sum_for_passback[k]; lmag+=P[n].BH_Specific_AngMom[k]*P[n].BH_Specific_AngMom[k];}
...
{
    int k; k=0; if(P[n].BH_Mdot <= 0) {P[n].BH_Mdot=0;} /* check unphysical values */
    ...
    double dm_acc_for_j = P[n].BH_Mdot * dt, m_tot_for_j = P[n].BH_Mass;
#ifdef BH_ALPHADISK_ACCRETION
    dm_acc_for_j = BlackholeTempInfo[i].mdot_alphadisk * dt; m_tot_for_j = P[n].BH_Mass + P[n].BH_Mass_AlphaDisk;
#endif
    for(k=0;k<3;k++) {P[n].BH_Specific_AngMom[k] = (m_tot_for_j*P[n].BH_Specific_AngMom[k] + dm_acc_for_j*BlackholeTempInfo[i].Jgas_in_Kernel[k]/(MIN_REAL_NUMBER + BlackholeTempInfo[i].Mgas_in_Kernel)) / (m_tot_for_j + dm_acc_for_j);}
#endif
...
/*  for BH_WIND_CONTINUOUS or BH_WIND_SPAWN
    ...
    double dMBH_continuous_accretion; dMBH_continuous_accretion = P[n].BH_Mdot * dt;
#ifdef BH_ALPHADISK_ACCRETION
    P[n].BH_Mass += dMBH_continuous_accretion;   // mdot comes from the disk - no mass loss here regardless of BAL model -
    double dm_alphadisk = BlackholeTempInfo[i].mdot_alphadisk * dt - dMBH_continuous_accretion;
    if(dm_alphadisk < -P[n].BH_Mass_AlphaDisk) {dm_alphadisk=-P[n].BH_Mass_AlphaDisk; P[n].BH_Mass_AlphaDisk=0;} else {P[n].BH_Mass_AlphaDisk += dm_alphadisk;}
    if(P[n].BH_Mass_AlphaDisk<0) {P[n].BH_Mass_AlphaDisk=0;}
    if(P[n].Mass<0) {P[n].Mass=0;}
    dMBH_continuous_accretion += dm_alphadisk;
#else // #ifdef BH_ALPHADISK_ACCRETION
#if defined(BH_WIND_CONTINUOUS) || defined(BH_WIND_SPAWN)
    P[n].BH_Mass += dMBH_continuous_accretion / All.BAL_f_accretion; // accrete the winds first, then remove the wind mass in the final loop
#else
    P[n].BH_Mass += dMBH_continuous_accretion;
#endif
#endif // #else BH_ALPHADISK_ACCRETION
#if defined(BH_SWALLOWGAS) && !defined(BH_GRAVCAPTURE_GAS)
    P[n].BH_AccretionDeficit += dMBH_continuous_accretion; // this is mass continuously accreted, which needs to be stochastically 'caught up to'
#endif
#ifdef JET_DIRECTION_FROM_KERNEL_AND_SINK
    double mtot = BlackholeTempInfo[i].Mgas_in_Kernel + P[n].Mass;
    ...
    for(k=0; k<3; k++) {
        ...
        P[n].Jgas_in_Kernel[k] = BlackholeTempInfo[i].Jgas_in_Kernel[k] + P[n].Mass * P[n].BH_Specific_AngMom[k]; // this stores the _total_ angular momentum (sink + gas) in the COM frame of the sink-gas system, including internal to the sink
    }
    P[n].Mgas_in_Kernel=BlackholeTempInfo[i].Mgas_in_Kernel;
#endif
...
void set_blackhole_drag(int i, int n, double dt)
{
    ...
    if((dt>0)&&(P[n].BH_Mass>0))
    {
        double fac = P[n].BH_Mdot * dt / P[n].BH_Mass;
#if (BH_DRAG == 2)
        double meddington = bh_eddington_mdot(P[n].BH_Mass);
        fac = meddington * dt / P[n].BH_Mass; /* make the force stronger to keep the BH from wandering */
#endif
        if(fac>1) fac=1;
    ...
    if(BlackholeTempInfo[i].DF_mmax_particles>0) /* found something in the kernel, we can proceed */
    {
...
#endif
        //fac = BlackholeTempInfo[i].Malt_in_Kernel / ( (4*M_PI/3) * pow(P[n].Hsml*All.cf_atime,3) ); /* mean density of all mass inside kernel */
        fac = Mass_in_Kernel / ( (4*M_PI/3) * pow(P[n].Hsml*All.cf_atime,3) ); /* mean density of all mass inside kernel */
        ...
        if(bhvel2_df > 0 && dt > 0)
        {
            ...
            afac_vel = All.G * Mass_in_Kernel / pow(P[n].Hsml*All.cf_atime,2); // GMenc/r^2 estimate of local acceleration //
            afac_vel = DMIN(dv_magnitude*UNIT_TIME_IN_MYR , DMAX( DMIN(DMAX(-2.*P[n].BH_MinPot/(P[n].Hsml*All.cf_atime*All.cf_atime), 0), 10.*dv_magnitude/dt), afac_vel)); // free-fall-acceleration [checked-to-zero], limited to multiple of actual vel difference in timestep
...
int bhsink_isactive(int i)
{
    ...
    if(P[i].Hsml <= 0) {return 0;} // only H>0, i.e. found neighbors
...
void blackhole_final_operations(void)
{
...
#endif
                double dr_min=0; for(k=0;k<3;k++) {dr_min+=(P[n].BH_MinPotPos[k]-P[n].Pos[k])*(P[n].BH_MinPotPos[k]-P[n].Pos[k]);}
                if(dr_min > 0 && dt > 0)
                {
                    ...
                    double dv_shift = sqrt(DMAX(-2.*P[n].BH_MinPot/All.cf_atime , 0)); // free-fall velocity, in [physical] code units, capped zero
...
#endif
                for(k = 0; k < 3; k++) {P[n].Pos[k] += (P[n].BH_MinPotPos[k]-P[n].Pos[k]) * fac_bh_shift;}
            }
#endif
    ...
    for(i=0; i<N_active_loc_BHs; i++)
    {
        ...
        if(update_bh_moments)
        {
            ...
            for(k=0;k<3;k++) {P[n].BH_Specific_AngMom[k] = (P[n].BH_Specific_AngMom[k]*P[n].Mass + BlackholeTempInfo[i].accreted_J[k]) / m_new;}
            P[n].BH_Specific_AngMom[0] -= (BlackholeTempInfo[i].accreted_centerofmass[1]*BlackholeTempInfo[i].accreted_momentum[2] - BlackholeTempInfo[i].accreted_centerofmass[2]*BlackholeTempInfo[i].accreted_momentum[1]) / (m_new*m_new);
            P[n].BH_Specific_AngMom[1] -= (BlackholeTempInfo[i].accreted_centerofmass[2]*BlackholeTempInfo[i].accreted_momentum[0] - BlackholeTempInfo[i].accreted_centerofmass[0]*BlackholeTempInfo[i].accreted_momentum[2]) / (m_new*m_new);
            P[n].BH_Specific_AngMom[2] -= (BlackholeTempInfo[i].accreted_centerofmass[0]*BlackholeTempInfo[i].accreted_momentum[1] - BlackholeTempInfo[i].accreted_centerofmass[1]*BlackholeTempInfo[i].accreted_momentum[0]) / (m_new*m_new);
#endif
...
#endif
            P[n].BH_Mass += BlackholeTempInfo[i].accreted_BH_Mass;
#ifdef BH_ALPHADISK_ACCRETION
            P[n].BH_Mass_AlphaDisk += BlackholeTempInfo[i].accreted_BH_Mass_alphadisk;
#endif
#ifdef GRAIN_FLUID
            P[n].BH_Dust_Mass += BlackholeTempInfo[i].accreted_dust_Mass;
#endif
#ifdef RT_REINJECT_ACCRETED_PHOTONS
	    P[n].BH_accreted_photon_energy += BlackholeTempInfo[i].accreted_photon_energy;
#endif
        } // if(masses > 0) check
#ifdef HERMITE_INTEGRATION
...
#endif
        double radiation_loss = evaluate_blackhole_radiative_efficiency(P[n].BH_Mdot,P[n].BH_Mass,n) * dm;
        if(radiation_loss > DMIN(P[n].Mass,P[n].BH_Mass)) radiation_loss = DMIN(P[n].Mass,P[n].BH_Mass);
#ifdef SINGLE_STAR_STARFORGE_PROTOSTELLAR_EVOLUTION
        if(All.BlackHoleRadiativeEfficiency > 0 && All.BlackHoleRadiativeEfficiency < 1 && P[n].ProtoStellarStage != 7) {radiation_loss = 0;} // negligible radiation loss term unless the object is actually a compact relic
#endif
#ifndef BH_DEBUG_FIX_MDOT_MBH
        P[n].Mass -= radiation_loss; P[n].BH_Mass -= radiation_loss;
#endif
        /* subtract the BAL wind mass from P[n].Mass && (P[n].BH_Mass || P[n].BH_Mass_AlphaDisk) // DAA: note that the mass loss in winds for BH_WIND_KICK has already been taken into account */
#ifdef BH_WIND_CONTINUOUS
        double dm_wind = (1.-All.BAL_f_accretion) / All.BAL_f_accretion * dm;
        if(dm_wind > P[n].Mass) {dm_wind = P[n].Mass;}
#ifdef BH_ALPHADISK_ACCRETION
        if(dm_wind > P[n].BH_Mass_AlphaDisk) {dm_wind = P[n].BH_Mass_AlphaDisk;}
        P[n].Mass -= dm_wind;
        P[n].BH_Mass_AlphaDisk -= dm_wind;
#else
        if(dm_wind > P[n].BH_Mass) {dm_wind = P[n].BH_Mass;}
#ifndef BH_DEBUG_FIX_MDOT_MBH
        P[n].Mass -= dm_wind; P[n].BH_Mass -= dm_wind;
#endif
#endif
       ...
       if(P[n].ProtoStellarStage == 5) {
        ...
        if(P[n].ProtoStellarStage == 6) { // Star old enough to go out with a boom
            double eps = DMIN(KERNEL_CORE_SIZE*ForceSoftening_KernelRadius(n), P[n].Hsml);
#ifdef BH_GRAVCAPTURE_FIXEDSINKRADIUS
            ...
            dm_wind = DMIN(SN_mdot*dt, P[n].BH_Mass); // We will spawn particles to model the SN ejecta, but not more than what we can handle at the same time, these particles will have the same mass as gas particles, not like wind particles
            ...
            P[n].BH_Mass -= dm_wind; // remove amount of mass lost via winds
            double m_relic = single_star_relic_SN_mass(n); // get the relic mass that should be left behind after the SNe
            if(P[n].BH_Mass < m_relic + 0.5*m_ejecta_unit) { // less than half a shell mass left in the star: instead of spawning the last shell with very low mass particles we will make the one before slightly more massive
                dm_wind += P[n].BH_Mass; // add leftover mass to be spawned and zero-out mass
                P[n].BH_Mass = DMAX(0, m_relic); // zero out mass if nothing is left-over
            }
            if(P[n].BH_Mass <= m_relic) {
        ...
        double n_unspawned = P[n].unspawned_wind_mass / ((BH_WIND_SPAWN)*target_mass_for_wind_spawning(n)); // number of spawned gas cells that can be made from the mass in the reservoir
        ...
        mass_disk=0; mdot_disk=0; MgasBulge=0; MstarBulge=0; r0 = P[n].Hsml * All.cf_atime;
#ifdef BH_ALPHADISK_ACCRETION
        mass_disk = P[n].BH_Mass_AlphaDisk;
        ...
        fprintf(FdBlackHolesDetails, "%.16g %llu  %g %g %g %g %g  %g %g %g %g %g %g  %2.16g %2.16g %2.16g  %2.16g %2.16g %2.16g  %g %g %g  %g %g %g\n",
                All.Time, (unsigned long long)P[n].ID,  P[n].Mass, P[n].BH_Mass, mass_disk, P[n].BH_Mdot, mdot_disk, dt, P[n].DensAroundStar*All.cf_a3inv, BlackholeTempInfo[i].BH_InternalEnergy,
                ...
                BlackholeTempInfo[i].Jgas_in_Kernel[0], BlackholeTempInfo[i].Jgas_in_Kernel[1], BlackholeTempInfo[i].Jgas_in_Kernel[2], P[n].BH_Specific_AngMom[0]*P[n].Mass, P[n].BH_Specific_AngMom[1]*P[n].Mass, P[n].BH_Specific_AngMom[2]*P[n].Mass ); fflush(FdBlackHolesDetails);
#else
        fprintf(FdBlackHolesDetails, "%.16g %llu  %g %g %g %g %g %g  %g %g %g %g %g %g %g %g  %2.16g %2.16g %2.16g  %2.16g %2.16g %2.16g  %g %g %g  %g %g %g\n",
                All.Time, (unsigned long long)P[n].ID,  P[n].Mass, P[n].BH_Mass, mass_disk, P[n].BH_Mdot, mdot_disk, dt, P[n].DensAroundStar*All.cf_a3inv, BlackholeTempInfo[i].BH_InternalEnergy, BlackholeTempInfo[i].Sfr_in_Kernel,
...
#ifdef OUTPUT_ADDITIONAL_RUNINFO
        fprintf(FdBlackHolesDetails, "BH=%llu %.16g %g %g %g %g %g %g %g   %2.16g %2.16g %2.16g\n", (unsigned long long)P[n].ID, All.Time, P[n].BH_Mass, mass_disk, P[n].Mass, P[n].BH_Mdot, mdot_disk,
        ...
        if(P[n].BH_Mass > 0) {TimeBin_BH_Medd[bin] += P[n].BH_Mdot / P[n].BH_Mass;}
...
/* check promotion and clipping flags */
        ...
        if((All.ComovingIntegrationOn==1) && (All.Time>All.TimeBegin) && (P[n].Type==5) && (P[n].Hsml>=0.5*All.BlackHoleMaxAccretionRadius/All.cf_atime)
           && (P[n].NumNgb<=0) && (BlackholeTempInfo[i].Mgas_in_Kernel + BlackholeTempInfo[i].Mstar_in_Kernel <= 0)) {P[n].Mass=0;} // clip BH - if near max radius and sees no gas -or- star particles, basically can't be in high-res region (or at least in a galaxy)
#endif
