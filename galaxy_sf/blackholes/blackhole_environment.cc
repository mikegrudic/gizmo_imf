            ...
            for(n = 0; n < numngb; n++)
            {
                ...
                if( (P[j].Mass > 0) && (P[j].Type != 5) && (P[j].ID != local.ID) )
                {
                    ...
                    if(P[j].Type==0)
                    {
                        ...
                        out.BH_InternalEnergy += wt*CellP[j].InternalEnergy;
...
static inline void INPUTFUNCTION_NAME(struct INPUT_STRUCT_NAME *in, int i, int loop_iteration)
{
    int k, j_tempinfo = P[i].IndexMapToTempStruc; in->Hsml = P[i].Hsml; /* link to the location in the shared structure where this is stored */
