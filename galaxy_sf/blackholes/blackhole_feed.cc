...
static inline void INPUTFUNCTION_NAME(struct INPUT_STRUCT_NAME *in, int i, int loop_iteration)
{
    ...
    in->Hsml = P[i].Hsml; in->Mass = P[i].Mass; in->BH_Mass = P[i].BH_Mass; in->ID = P[i].ID; in->Density = P[i].DensAroundStar; in->Mdot = P[i].BH_Mdot;
#ifdef BH_GRAVCAPTURE_FIXEDSINKRADIUS
...
#endif
#ifdef BH_REPOSITION_ON_POTMIN
    if(mode==0) {P[i].BH_MinPot=out->BH_MinPot; for(k=0;k<3;k++) {P[i].BH_MinPotPos[k]=out->BH_MinPotPos[k];}
        } else {if(out->BH_MinPot < P[i].BH_MinPot) {P[i].BH_MinPot=out->BH_MinPot; for(k=0;k<3;k++) {P[i].BH_MinPotPos[k]=out->BH_MinPotPos[k];}}}
#endif
}
        ...
        while(startnode >= 0) {
            ...
            for(n = 0; n < numngb; n++)
            {
                ...
                if(P[j].Mass > 0)
                {
                    ...
                    double heff_j = DMAX( P[j].Hsml , ForceSoftening_KernelRadius(j) );
                    if(r2 < h_i2 || r2 < heff_j*heff_j)
                    {
                        ...
                        {
                            if(((local.ID != P[j].ID) || (r2>0)) && (SwallowID_j == 0) && (P[j].BH_Mass < local.BH_Mass)) /* we'll assume most massive BH swallows the other - simplifies analysis and ensures unique results */
                            {
                                ...
                                if(r >= heff_j) {allow_bh_merger = 0;} // beyond MAX[search/kernel/softening] radius: heff_j = DMAX( P[j].Hsml , ForceSoftening_KernelRadius(j) )
                                ...
                                max_rmerge = DMAX(max_rmerge, DMIN(local.Hsml, P[j].Hsml));
                        ...
                        if(P[j].Type == 0) /* here we have a gas particle */
                        {
                            ...
                            if(local.Density > 0) {
                                #pragma omp atomic
                                CellP[j].Injected_BH_Energy += (wk/local.Density) * energy * P[j].Mass;
                            }
#endif                            
                        } // if(P[j].Type == 0)
