...
#define TEST_ALIM 1
...
#else
#if (SLOPE_LIMITER_TOLERANCE != 2) && !(HYDRO_FIX_MESH_MOTION >= 5) // unless using the most aggressive reconstruction, we will limit face-area disparity here //
...
#if defined(HYDRO_TENSOR_FACE_CORRECTIONS) /* experimental! not really ready for full use yet! */
    {
        ...
        for(k=0;k<3;k++) {Bi_dot_dx += local.Tensor_MFM_Face_Corrections[k] * kernel.dp[k]; Bj_dot_dx -= CellP[j].Tensor_MFM_Face_Corrections[k] * kernel.dp[k];}
        ...
        Face_Area_Vec[0]+= wkwt_j * (CellP[j].Tensor_MFM_Face_Corrections[3]*kernel.dp[0] + CellP[j].Tensor_MFM_Face_Corrections[4]*kernel.dp[1] + CellP[j].Tensor_MFM_Face_Corrections[5]*kernel.dp[2]);
        Face_Area_Vec[1]+= wkwt_j * (CellP[j].Tensor_MFM_Face_Corrections[4]*kernel.dp[0] + CellP[j].Tensor_MFM_Face_Corrections[6]*kernel.dp[1] + CellP[j].Tensor_MFM_Face_Corrections[7]*kernel.dp[2]);
        Face_Area_Vec[2]+= wkwt_j * (CellP[j].Tensor_MFM_Face_Corrections[5]*kernel.dp[0] + CellP[j].Tensor_MFM_Face_Corrections[7]*kernel.dp[1] + CellP[j].Tensor_MFM_Face_Corrections[8]*kernel.dp[2]);
...
#if defined(HYDRO_FACE_AREA_LIMITER)
