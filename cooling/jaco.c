...
void call_jaco(int i) {
    /* Top-level entry point called by GIZMO's cooling routine for each particle.
       Runs the implicit solve, updates predicted energy, and sets pressure. */
    jaco_do_cooling(i);
...
void jaco_do_cooling(int i) {
    /* Run the implicit microphysics solve for particle i. */
    ...
    int num_iter = jaco_solve(&sv, &pr, 1e-6);
    jaco_to_gizmo(i, &sv);
}

void jaco_set_eos_pressure(int i) {
    /* EOS pressure update — currently ideal gas only.
       TODO: must eventually be codegen'd for the general EOS. */
    SphP[i].Pressure = (GAMMA(i) - 1) * SphP[i].InternalEnergyPred * Get_Gas_density_for_energy_i(i);
}

/* ---- Newton-Raphson solver ---- */

int iter_condition(SolveVars *sv, SolveVars *dsv, double tol) {
...
int solve_failure_condition(SolveVars *sv, SolveVars *dsv, double tol, int num_iter) {
